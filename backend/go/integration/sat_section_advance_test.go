...
func TestSATSectionAdvanceOpensTheBreakAtTheRuntimeClock(t *testing.T) {
	...
	t.Run("candidate-length clock opens the break at the authored boundary", func(t *testing.T) {
		...
		before := f.sweep("one second before the authored deadline", scheduleID, anchor.Add(64*time.Minute-time.Second), anchor)
		if before.section("reading-writing").Status != "live" || before.Waiting {
			t.Errorf("one second before the authored deadline the section must still be live and no break open, got %s waiting=%v",
		...
		at := f.sweep("authored deadline", scheduleID, anchor.Add(64*time.Minute), anchor)
		rw := at.section("reading-writing")
		if rw.Status != "completed" {
			t.Fatalf("at the authored boundary the section must complete, got %s", rw.Status)
		}
		if !at.Waiting {
	...
	t.Run("migration 0067 repairs a not-yet-started inflated section", func(t *testing.T) {
		...
		early := f.sweep("Math: one second before the deadline", scheduleID, mathStart.Add(70*time.Minute-time.Second), anchor)
		if early.section("math").Status != "live" {
			t.Errorf("the repaired Math clock must remain live until its 70-minute deadline, got %s", early.section("math").Status)
		}
		end := f.sweep("Math: authored deadline (runtime ends)", scheduleID, mathStart.Add(70*time.Minute), anchor)
