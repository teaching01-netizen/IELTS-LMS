...
// ensureACTGradingSession creates the queue parent alongside the canonical
// ACT submission. Keeping this in the terminalization transaction means a
// newly completed ACT attempt is visible to Grading immediately; the ACT
// repair worker has the same idempotent operation for older local rows.
