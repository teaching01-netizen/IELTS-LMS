  ...
  return page.evaluate(
    async ({ scheduleId, candidateId }) => {
      ...
      if (!payload.attempt?.id)
        throw new Error("The authenticated session did not return an attempt id.");
      return {
...
test.describe("SAT automatic resume", () => {
  ...
  test("recovers the same server attempt across a new tab, Student Link, and missing local storage", async ({
    ...
    const { studentContext, studentPage, scheduleId, candidateId, joinHref } =
      await createRunningSatSession(browser, page, { label: "auto-resume" });
    try {
      ...
      const cachedAttemptA = await studentPage.evaluate(() =>
      ...
      await expect(schedulePage.locator('input[type="radio"]').first()).toBeChecked({
        timeout: 30_000,
      });
      ...
      await expect(secondStudentPage).toHaveURL(new RegExp(`/student/${scheduleId}/[^/]+$`), {
        timeout: 30_000,
      });
      const secondCandidateId = decodeURIComponent(
        new URL(secondStudentPage.url()).pathname.split("/").at(-1) ?? ""
      );
      expect(secondCandidateId).not.toBe(candidateId);
      await expect(secondStudentPage.getByTestId("sat-exam-shell")).toBeVisible({
        timeout: 45_000,
      });
      const secondAttempt = await readAttemptIdentity(
        secondStudentPage,
        scheduleId,
        secondCandidateId
      );
      ...
      await schedulePage.evaluate(
        ({ secondCandidateId, secondAttemptId }) => {
      ...
      await schedulePage.goto(`/student/${scheduleId}`, { waitUntil: "domcontentloaded" });
      await expect(schedulePage.getByTestId("sat-exam-shell")).toBeVisible({ timeout: 45_000 });
      await expect(schedulePage).toHaveURL(
        new RegExp(`/student/${scheduleId}/${encodeURIComponent(candidateId)}$`)
      );
      ...
      await expect(linkPage.locator('input[type="radio"]').first()).toBeChecked({
        timeout: 30_000,
      });
      ...
      await restartedPage.evaluate(
        ({ scheduleId, candidateId, attemptId, cachedAttempt }) => {
          window.localStorage.setItem(
            "sat-resume-locator:v1",
            JSON.stringify({
              version: 1,
              providerKey: "sat",
              scheduleId,
      ...
      await restartedPage.goto(`/student/${scheduleId}`, { waitUntil: "domcontentloaded" });
      await expect(restartedPage.getByRole("heading", { name: "Exam Check-in" })).toBeVisible({
        timeout: 30_000,
      });
      await expect(restartedPage.getByTestId("sat-exam-shell")).toHaveCount(0);
      await restartedPage.goto(`/student/${scheduleId}/${encodeURIComponent(candidateId)}`, {
        waitUntil: "domcontentloaded",
      });
      await expect(restartedPage.getByTestId("sat-exam-shell")).toHaveCount(0);
      ...
      await linkPage.evaluate(
        ({ scheduleId, candidateId, attemptId }) => {
          window.localStorage.setItem(
            "sat-resume-locator:v1",
            JSON.stringify({
              version: 1,
      ...
      const resumeRoute = async (route: import("@playwright/test").Route) => {
        ...
        if (
          url.pathname === `/api/v1/student/sessions/${scheduleId}` &&
          url.searchParams.has("refreshAttemptCredential")
        ) {
          resumeProbes += 1;
          if (resumeProbes === 1) {
      ...
      await expect(
        offlinePage.getByRole("heading", { name: "We couldn’t reconnect to your SAT" })
      ).toBeVisible({ timeout: 45_000 });
      expect(resumeProbes).toBe(1);
      await expect
        .poll(() =>
          offlinePage.evaluate(() => window.localStorage.getItem("sat-resume-locator:v1"))
        )
        .not.toBeNull();
      ...
      await offlinePage.goto(`/student/${scheduleId}`, { waitUntil: "domcontentloaded" });
      await expect(offlinePage.getByTestId("sat-exam-shell")).toBeVisible({ timeout: 45_000 });
      const resumedQuestionName = await offlinePage
        .locator('input[type="radio"]')
        .first()
        .getAttribute("name");
      if (!resumedQuestionName)
        throw new Error("SAT answer control did not expose its question group.");
      const resumedQuestionChoices = offlinePage.locator(
        `input[type="radio"][name="${resumedQuestionName}"]`
      );
      ...
      if (!replacementChoiceId)
        throw new Error("SAT answer control did not expose its associated label.");
      ...
      try {
        await expect(
          terminatedPage.getByRole("heading", { name: "Your SAT attempt has ended" })
        ).toBeVisible({ timeout: 45_000 });
      } catch (error) {
        throw new Error(
          `Terminated reopen did not show the terminal surface: ${JSON.stringify({
            url: terminatedPage.url(),
            text: await terminatedPage
              .locator("body")
              .innerText()
              .catch((readError) => String(readError)),
      ...
      await expect
        .poll(() =>
          terminatedPage.evaluate(() => window.localStorage.getItem("sat-resume-locator:v1"))
        )
        .toBeNull();
      ...
      await expect(unauthenticatedPage.getByRole("heading", { name: "Exam Check-in" })).toBeVisible(
        { timeout: 30_000 }
      );
  ...
  test("reopens from proctor pause and section break while the student is away", async ({
    ...
    try {
      ...
      await expect(pausedPage.getByRole("heading", { name: "Your timer is paused" })).toBeVisible({
        timeout: 30_000,
      });
      ...
      await expect(pausedPage.getByTestId("sat-exam-shell")).toBeVisible({ timeout: 45_000 });
      await expect(pausedPage.getByRole("heading", { name: "Your timer is paused" })).toHaveCount(
        0
      );
      ...
      await expect
        .poll(async () => (await readRuntimeSnapshot(scheduleId)).currentSectionKey, {
          timeout: 60_000,
      ...
      try {
        ...
        throw new Error(
          `Math reopen did not mount the SAT shell: ${JSON.stringify({
            ...
            rendered,
            runtime: await readRuntimeSnapshot(scheduleId),
            attempt: await readAttemptIdentity(mathPage, scheduleId, candidateId).catch(
      ...
      expect(await readAttemptIdentity(mathPage, scheduleId, candidateId)).toMatchObject({
        id: original.id,
      });
