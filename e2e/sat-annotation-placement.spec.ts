...
function expectCentredOnAnchoredLine(geometry: PlacementGeometry): void {
  ...
  expect(geometry.surface.right, 'the bar sits clear of the line it acts on')
    .toBeGreaterThanOrEqual(line.left - 1);
  expect(geometry.surface.left, 'the bar sits clear of the line it acts on')
    .toBeLessThanOrEqual(line.right + 1);
  ...
  const moved = geometry.surface.left <= regionLeft + 2 || geometry.surface.right >= regionRight - 2;
  if (moved) return;

  const barCentre = (geometry.surface.left + geometry.surface.right) / 2;
  expect(Math.abs(barCentre - (line.left + line.right) / 2), 'the bar is centred on the line it acts on')
 ...
 */
async function readBar(page: Page): Promise<{ rowHeight: number; pointer: boolean; overflowX: number }> {
...
test.describe('annotation surface placement', () => {
  for (const profile of PROFILES) {
    test(`${profile.name}: the surface stays visible, clear of the selection, on the anchored line`, async ({ page, isMobile }) => {
