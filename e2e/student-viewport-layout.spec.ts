    ...
    .poll(
      async () => {
        ...
        const diagnostic = await page.evaluate(() => {
          const status =
            document.querySelector<HTMLElement>('[data-testid="student-auto-save-status"]')
              ?.textContent ?? null;
          const answer =
            document.querySelector<HTMLInputElement>('input[aria-label*="Answer for question"]')
              ?.value ?? null;
...
async function selectStudentQuestion(page: Page, questionNumber: number) {
  const layoutMode = await page
    ...
    .getAttribute("data-student-layout-mode");
  if (layoutMode === "compact") {
    const navigator = page.getByRole("button", { name: /^Open question navigator/ });
    const currentLabel = await navigator.getAttribute("aria-label");
    const currentQuestion = Number(currentLabel?.match(/question\s+(\d+)\s+of\s+\d+/i)?.[1]);
    if (!Number.isInteger(currentQuestion)) {
      throw new Error(
        `Cannot read current question from navigator label: ${currentLabel ?? "<missing>"}`
      );
    }

    for (let question = currentQuestion; question < questionNumber; question += 1) {
      const nextButton = page.getByRole("button", { name: "Next question" });
      await expect(nextButton).toBeEnabled();
      await nextButton.click();
    }
    for (let question = currentQuestion; question > questionNumber; question -= 1) {
      const previousButton = page.getByRole("button", { name: "Previous question" });
...
test.describe("student viewport layout acceptance", () => {
  ...
  test("AC-14: compact navigation boundaries never submit", async ({ browser }, testInfo) => {
    ...
    try {
      ...
      await expect(current).toHaveAttribute(
        "aria-label",
        expect.stringContaining("question 1 of 3")
      );

      await next.click();
      await expect(current).toHaveAttribute(
        "aria-label",
        expect.stringContaining("question 2 of 3")
      );
      ...
      await expect(current).toHaveAttribute(
        "aria-label",
        expect.stringContaining("question 3 of 3")
      );
  ...
  test("AC-15: tools and navigator stay reachable through a presentation change", async ({
    ...
    try {
      ...
      expect(navigatorBox!.y + navigatorBox!.height).toBeLessThanOrEqual(
