...
// The Library UI now lives at /admin/library ('Content Library'); the /admin/media
// route, Audio/Images tab structure, upload labels, and storage-budget testids
// asserted below no longer exist. Explicit skip (fail-closed); do not delete.
test.describe
  .skip("QUARANTINE (WS-16): Admin Media Library Management — selector drift, see WS-16", () => {
  ...
  test("manages passage library", async ({ page }) => {
    ...
    await page
      .getByLabel("Content")
      .fill(
        "This is a test passage for the passage library. It contains sample text that can be used in reading passages."
      );
