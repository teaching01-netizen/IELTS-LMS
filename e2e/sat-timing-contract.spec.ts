...
async function configureDeliveryThroughApi(
  page: Page,
  examId: string,
  title: string,
  values: {
...
async function runtimeFrom(page: Page, scheduleId: string): Promise<RuntimeSnapshot> {
  ...
  expect(response.status, JSON.stringify(response.payload)).toBe(200);
  const payload = unwrap<{ runtime: RuntimeSnapshot }>(
    response.payload as ApiPayload<{ runtime: RuntimeSnapshot }>
  );
...
async function openRuntimeWebSocket(page: Page, scheduleId: string): Promise<void> {
  await page.evaluate((id) => {
    ...
    const socket = new WebSocket(
      `${protocol}//${location.host}/api/v1/ws/live?scheduleId=${encodeURIComponent(id)}`
    );
...
test.describe("SAT timing contract against MySQL, HTTP, WebSocket, and browser routes", () => {
  ...
  test.skip(
    ...
    "The real timing contract runs once in Chromium."
  );
  ...
  test("keeps one proctor-anchored schedule through equal/unequal branches, break refreshes, and null boundaries", async ({
    ...
    try {
      ...
      const shellResponse = await readApi(
        page,
        `/api/v1/assessment-authoring/exams/${examId}/shell`
      );
      expect(shellResponse.status, JSON.stringify(shellResponse.payload)).toBe(200);
      const shellLifecycle = unwrap<AssessmentAuthoringShellResult>(
        shellResponse.payload as ApiPayload<AssessmentAuthoringShellResult>
      );
      if (!shellLifecycle.shell)
        throw new Error("New SAT did not expose an editable authoring shell.");
      ...
      await configureDeliveryThroughApi(page, examId, "Reading & Writing", {
        base: 5,
        lower: 5,
        higher: 5,
        breakMinutes: 5,
      });
      await configureDeliveryThroughApi(page, examId, "Math", {
        base: 5,
        lower: 3,
        higher: 7,
        breakMinutes: 0,
      });
      const validationResponse = await writeApi(
        page,
        "POST",
        `/api/v1/assessment-authoring/exams/${examId}/validate`
      );
      ...
      const finalShellResponse = await readApi(
        page,
        `/api/v1/assessment-authoring/exams/${examId}/shell`
      );
      ...
      if (!finalShellLifecycle.shell)
        throw new Error("Final SAT draft shell was unavailable before publish.");
      ...
      const publishedExam = unwrap<ExamSnapshot>(
        publishedExamResponse.payload as ApiPayload<ExamSnapshot>
      );
      expect(publishedExam.currentPublishedVersionId).toBeTruthy();

      const linkResponse = await writeApi(
        page,
        "POST",
        `/api/v1/assessment-access/exams/${examId}/links`,
        {
      ...
      const link = unwrap<AccessLinkSnapshot>(
        linkResponse.payload as ApiPayload<AccessLinkSnapshot>
      );
      ...
      const startResponse = await writeApi(
        page,
        "POST",
      ...
      const initialRuntime = await waitForRuntime(
        page,
        scheduleId,
      ...
      await expect
        .poll(
          async () =>
            (await readRuntimeFrames(wsPage!)).some((frame) => frame.type === "runtime_snapshot"),
          { timeout: 20_000, intervals: [250, 500, 1_000] }
        )
        .toBe(true);
      const snapshotFrame = (await readRuntimeFrames(wsPage)).find(
        (frame) => frame.type === "runtime_snapshot"
      );
      expect(
        snapshotFrame?.runtime?.sections.map((candidate) => candidate.plannedDurationMinutes)
      ).toEqual([10, 12]);
      expect(
        Object.prototype.hasOwnProperty.call(snapshotFrame?.runtime ?? {}, "nextSectionStartAt")
      ).toBe(true);
      ...
      const waitingRuntime = await waitForRuntime(
        ...
        scheduleId,
        (runtime) =>
          runtime.waitingForNextSection &&
          section(runtime, "reading-writing").status === "completed"
      );
      ...
      expect(new Date(waitingRuntime.nextSectionStartAt as string).getTime()).toBeGreaterThan(
        Date.now()
      );
      ...
      await expect
        .poll(
          async () =>
            (await readRuntimeFrames(wsPage!)).some(
              (frame) =>
      ...
      await expect(studentPage).toHaveURL(new RegExp(`/student/${scheduleId}/[^/]+$`), {
        timeout: 30_000,
      });
      ...
      const mathRuntime = await waitForRuntime(
        ...
        scheduleId,
        (runtime) =>
          runtime.activeSectionKey === "math" &&
          !runtime.waitingForNextSection &&
          section(runtime, "math").status === "live"
      );
  ...
  test("grants the full authored duration on the first active frame under sat_personal_v1", async ({
    ...
    const joinStudent = async (
      ...
      const startDelayed = new Promise<void>((resolve) => {
      ...
      if (delayFirstModuleStartMs > 0) {
        ...
        await context.route(
          "**/v1/assessment-delivery/schedules/*/modules/start",
          async (route) => {
            if (alreadyHeld) {
    ...
    try {
      ...
      const first = await joinStudent(
        link,
        linkName,
        `Personal A ${stamp}`,
        `sat-personal-a-${stamp}@example.com`,
        5_000
      );
      await first.startDelayed;
      await expect
        .poll(async () => (await first.studentPage.locator("h1").first().innerText()).trim(), {
          timeout: 30_000,
        })
      ...
      await expect(first.studentPage.getByTestId("sat-exam-shell")).toBeVisible({
        timeout: 45_000,
      });
      expect(await examTimer(first.studentPage)).toBe("2:00");
      await expect
        .poll(() => examTimer(first.studentPage), { timeout: 10_000, intervals: [250, 500] })
        .toBe("1:59");
      ...
      const second = await joinStudent(
        link,
        linkName,
        `Personal B ${stamp}`,
        `sat-personal-b-${stamp}@example.com`,
        0
      );
      await expect(second.studentPage.getByTestId("sat-exam-shell")).toBeVisible({
        timeout: 45_000,
      });
  ...
  test("gives two candidates who join five minutes apart their own authored windows", async ({
    ...
    try {
      ...
      const firstPage = await joinStudent(
        link,
        linkName,
        `Five A ${stamp}`,
        `sat-personal-five-a-${stamp}@example.com`
      );
      ...
      const secondPage = await joinStudent(
        link,
        linkName,
        `Five B ${stamp}`,
        `sat-personal-five-b-${stamp}@example.com`
      );
