...
test.describe("SAT answer durability recovery", () => {
  ...
  test("shows the last server-saved answer in staff Results after the student closes the exam", async ({
    page,
    browser,
  }) => {
    const { studentContext, studentPage, examTitle, linkName, studentName } =
      await createRunningSatSession(browser, page, { label: "staff-answers" });
    try {
  ...
  test("preserves answer, elimination, and annotation metadata through delayed recovery", async ({
    ...
    try {
      ...
      await expect(
        studentPage.getByRole("button", { name: "Submit module", exact: true })
      ).toHaveCount(0);
      await expect(
        studentPage.getByRole("button", { name: "Submit anyway", exact: true })
      ).toHaveCount(0);
