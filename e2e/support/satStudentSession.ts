...
export async function createRunningSatSession(
  ...
  await expect(
    adminPage
      .getByText(/147 of 147 (questions )?authored/)
      .filter({ visible: true })
      .first()
  ...
  await expect(adminPage.getByText("Saved", { exact: true }).last()).toBeVisible({
    timeout: 90_000,
  });
  ...
  try {
    ...
    throw new Error(
      `Could not open SAT release page after the room save barrier: ${JSON.stringify(details)}`,
      { cause: error }
    );
  }
  ...
  const joinHref = await adminPage
  ...
  try {
    ...
    const pageText = await studentPage
      .locator("body")
      .innerText()
      .catch(() => "Unable to read page text.");
    throw new Error(
      `Student Link page did not load at ${studentPage.url()}: ${pageText.slice(0, 1_000)}`,
      { cause: error }
    );
  }
  ...
  if (options.startRuntime !== false) {
    ...
    await expect(studentPage.locator('input[type="radio"]').first()).toBeVisible({
      timeout: 30_000,
    });
  }
