...
export function studentFlow(data) {
  ...
  const firstQuestionId = pickFirstQuestionId(contentSnapshot);
  const firstWritingTaskId = pickFirstWritingTaskId(contentSnapshot);
  ...
  const answerValue = `k6 ${runId} ${student.wcode} answer`;
  const writingValue = `k6 ${runId} ${student.wcode} writing`;
  ...
  if (!answerOk || !writingOk || !finalSubmissionOk) {
    ...
    fail(
      `Submitted attempt data mismatch for ${student.wcode}: answerOk=${answerOk} writingOk=${writingOk} finalSubmissionOk=${finalSubmissionOk}`,
    );
  }

  if (DEBUG) console.log(`[student ${student.wcode}] submit verified`);
}
