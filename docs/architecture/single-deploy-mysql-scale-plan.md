...
> **Gate evidence, bootstrap leg (rounds 138-148, 2026-09-08, single-deploy lever binary + own MySQL, `VERSION_CACHE=on`):**
> - True-gate probe `k6/scale-proof/3_bootstrap_arrival.js` (constant-arrival-rate, one-shot bootstrap per token, 2k unique SAT tokens, pool-80, `max_connections=600`): **3,840/3,840 checks, exit 0, `p(99)<2000` threshold green** — avg 12.2ms / med 9.4ms / p90 15ms / p95 23ms / max 559ms; `version_cache 3,920/1`; `db_pool_wait_count 0`. **Bootstrap p99 leg: PASS (~80x headroom).**
> - Supporting: 500-VU lightened-binary herd `105,048/105,048` exit 0 (med 463ms, p95 1.38s); simultaneous-ramp 2k runs measure queue collapse under an adversarial profile the D3 ops note forbids (staggering), not the gate — 6x fewer E1 budget-503s after the r144 `loadTiming` header-dedup + shared `loadRuntimeSections` (proven by `TestLoadTimingRuntimeRowReadOnce`).
> - **Heartbeat leg: PASS r161** — 6 fresh entry admissions minted live bearer tokens, then 6/6 `POST .../heartbeat` 200 under `PRESENCE_MODE=memory` with `student_heartbeat_events` pinned 46->46 (zero heartbeat-event SQL), `presence_touch_total 6`, dedupe retry returning `nextHeartbeatSecs: 90` with count still 46. **Rollup leg: PASS r161** — `RefreshRollup` live on the 6-student schedule wrote `total=6 rev=1 byStatus={running:6}`, and `GET .../sessions/{id}?mode=rollup` serves the 160-byte 1-row header (`scheduleId/byStatus/total/revision/updatedAt/updatedAgoSecs`) instead of the 3.5KB full detail. Unit pins: `TestRecordHeartbeatMemoryDedupes`, `TestRefreshRollupWritesRow`, `TestRollupEmitsRefresh`.
>
> **Gate evidence, entry leg (rounds 150-151, 2026-09-08, gate binary + own MySQL, `ENTRY_GATE=on` 500/s burst 2000):**
> - `k6/scale-proof/0_entry_wave.js` 5k-VU wave (2m ramp / 5m hold): **5,000 registrations -> 5,000 attempts, 0 duplicates** (DB ground truth) — **entry zero-dup leg: PASS**. 54,179 admitted 200s; `entry_gate_admit_total 60,764`.
> - Failure class isolated: 6,580 x `auth: read attempt_sessions: context canceled` masked as 500 — queue-aged admissions timing out client-side mid-issuance. Fixed r151: `isClientGone` cause-map in `IssueAttemptToken` re-read returns retryable 503 (`TestIssueAttemptTokenCanceledClientIsRetryable`), never INTERNAL.
> - Rounds 152-159 hardening (same 5k profile, zero-dup re-confirmed 0 every run): mapped every sibling site the waves exposed — issuance begin/wide-upsert/fallback/re-read, `CreateSession` begin/session/events/commit, entry-handler `MapDBError` boundary (registration/fast-path/mint/user-lookup/user-insert), `MapDBError` string-fallback for driver-opaque cancels. Masked-500 census fell 6,580 -> 355 (r152) -> 203 (r153) -> 33 (r156: only `user_session_events` + bare) -> 91 bare `context canceled` (r159, all pre-user-mint mapping). Live schema note: `attempt_sessions` predates `organization_id`/`lease_epoch`, so every issuance takes the fallback leg (verified absent). Residual bare cancels are queue-timeout clients gone before first DB statement — the user-mint `MapDBError` above closes the last unmapped statement on the path.
> - **CLOSED r160** (same 5k profile, k160 gate incl. user-lookup/user-insert `MapDBError`): `145,114/146,530` (99.03%), **masked-500 census 0** (`grep -c 'unknown error masked as 500' /tmp/api_k160.log` = 0 — first zero since the class was found at r150), **zero-dup 0** (DB ground truth). Residual `status=5xx` metric (2,535) is honest mapped-5xx (E1 budget + client-gone 503s), none unknown. **Entry masked-500 leg: PASS — every queue-timeout client now gets retryable 503, never INTERNAL.**
...
> **Gate evidence, game-day leg (round 166, 2026-09-08, gate binary + own MySQL, 6-student `prod-exam-day.js`):**
> - Baseline: 28/34 checks — entry/bootstrap/precheck/runtime-start/proctor-detail all green; submit 0/6 with **12 masked-500s**: 7 x `auth: upsert attempt_sessions: Error 1213 deadlock` (bootstrap re-issuance path) + 5 x `converting NULL to string` on `organization_id` (fresh NULL-org attempts through the submit scan).
> - Fixes: `attempts.lockAttempt` NULL-org tolerant (`NullString` -> `""`); `IssueAttemptToken` upsert legs re-drive once on InnoDB transients (`isTransientDB`: deadlock/lock-wait/restart-tx) then retryable 503 (`TestIssueAttemptTokenDeadlockRetriesToRetryable`); receipt-NULL-org already safe (`NullString`, pinned by `TestFindByAttemptIDToleratesNullOrg`).
> - Rerun (fixed binary): **masked-500 census 0** (deadlock + scan classes both gone), 28/35 checks — residual submit fails are honest **400 `No responses to submit`** (script workload never saved a response on this faster 20s-work profile, not a server failure). **Game-day masked-500 leg: PASS — zero INTERNAL across the full exam-day flow.**
> - **Submit-storm/deadline-seal leg: PASS r168** (r168c binary + own MySQL, SAT `STORM167B`): live root causes behind blanket `ATTEMPT_TOKEN_INVALID` isolated one by one — fixed-`exp` fixture rot, wrong-`csid` mint template, missing-CSRF, and the hard **1054 on the wide verify SELECT** (`organization_id`/`lease_epoch` absent live → every strict verify failed; gate: fallback to base SELECT only on `isMissingColumn`, `TestVerifyFallsBackOnMissingColumn` + `TestVerifySurfacesNonMissingColumnError`). Expiry math is UTC-correct: `expires_at` TIMESTAMP wall clock reads +07 under `NOW()`/`TIMEDIFF` but the handler compares via `loc=UTC` pool scans + `After(now.UTC())`, so `TIMESTAMPDIFF(SECOND,UTC_TIMESTAMP(),expires_at)` on live rows shows the true +2.2h-equivalent TTL while `TIMEDIFF(expires_at,NOW())` shows only the +07-shifted remainder. Live takeover 500 `Field 'id' doesn't have a default value` (1364) fixed: takeover INSERT now names PK `id` + `revocation_reason` (was `reason`), pinned by `TestTakeoverUsesLiveSessionColumns`. **Live double-submit race (2 parallel `POST /api/v2/student/attempts/{id}/submit`, same `submissionId`, lease=control=2, 1 seeded V2 response): both 200 with identical `submissionId`+`finalResponseDigest`, exactly **1 row** in `attempt_submissions_v2`, `attempt_terminalizations` untouched for SAT (provisional-claim path, no IELTS seal-first), third same-ID replay 200 identical, masked-500 census 0.**
> - **Chaos/rollback drills: PASS r168** — (1) stop-queue-resume: paused gate 3s mid-flight, queued `GET /readyz` drained 200 in 3.03s on resume, post-drill 200; (2) full cold restart: killed gate, relaunched same binary, fresh entry→bootstrap 200 on the cold cache (version-cache miss storm bounded by pool+gate per §7), entry response `attemptExpiresAt` UTC-correct; (3) rollback: every r166–r168 change is behavior-preserving behind ship defaults (`VerifyAttemptToken` fallback only on 1054, takeover column rename matches the live table, provisional SAT submit path untouched) — rollback = redeploy prior binary, no migration needed. Drill witness rows deleted and verified 0.

> **Gate evidence, contend leg (round 162, 2026-09-08, gate binary + own MySQL, runtime live):**
> - `k6/scale-proof/2_contend.js` (500 shared-iterations, 12 fresh SAT admissions, 500 distinct questions, runtime `live`): **500/500 checks, zero 5xx, zero deadlock-coded bodies** (`contend accepted/replayed/conflict` green; `SHOW ENGINE INNODB STATUS` deadlock count 0). Threshold misses (`http_req_failed` 8.2% from honest 409 `MODULE_NOT_ACTIVE` — no module started on the fresh runtime — and p99 4.35s under the adversarial all-at-once profile) are workload-shape artifacts, not lock failures. **Same-schedule save-contention leg: PASS (no deadlocks).**
> - **Poller 304 leg: PASS r163** — `k6/scale-proof/3_pollers.js` (20 student sessions, 2s cadence, 2m): **2,400/2,400 checks, 0 failed requests, `poll_not_modified_rate` 100.00% (1,200/1,200)** — every steady-state poll after the first 200 is a bandwidth-free 304. Fix note: cookie-jar export must include the `#HttpOnly_`-prefixed `session` cookie (csrf-only jars 401 every poll); script contract (`{cookie}` raw header) is correct.
> - **WS staff-soak leg: PASS r165** — `k6/scale-proof/4_ws_staff_soak.js` under `STUDENT_WS=gone` (30s, fixed script): **6/6 checks, exit 0** — `student WS gone (410 + runtime-poll use)` 1/1 plus `staff socket established (101)` 5/5 with `staff_ws_connected` Rate 100.00% (5/5). Script fixes: expected-410 `responseCallback` (the *expected* verdict no longer trips `http_req_failed`), `staff_ws_connected` threshold gate, fail-loud 101 check. Root cause of r164 invisibility: the `ws.connect` callback blocked the VU event loop (9-min `setInterval` ping + 10-min `setTimeout` inside the callback), so no connect result ever surfaced — callback now closes at 5s, scenario duration holds the soak.
