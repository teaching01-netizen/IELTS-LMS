...
describe("studentAttemptRepository", () => {
  ...
  it("fails closed when pending mutations cannot reach any durable browser store", async () => {
    const attempt = makeAttempt();
    await studentAttemptRepository.saveAttempt(attempt);

    const originalIndexedDb = (window as Window & { indexedDB?: IDBFactory }).indexedDB;
    ...
    try {
      ...
      ).rejects.toThrow(/Unable to durably store pending student mutations/i);
  ...
  it("does not claim volatile-memory fallback as durable when both browser stores fail", async () => {
  ...
  it("prunes submitted synced attempts to receipts and purges old receipts", async () => {
    ...
    expect(window.localStorage.getItem("ielts_student_attempt_receipts_v1")).toContain(
      "submission-1"
    );
