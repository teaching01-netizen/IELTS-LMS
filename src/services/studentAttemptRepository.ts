...
const inMemoryClientSessionIds = new Map<string, string>();
...
function ensureClientSessionId(
  ...
  if (stored && !forceNew) {
    inMemoryClientSessionIds.delete(storageKey);
    try {
  ...
  if (!forceNew && inMemoryClientSessionIds.has(storageKey)) {
    const remembered = inMemoryClientSessionIds.get(storageKey)!;
    let persisted = false;
    try {
      session?.setItem(storageKey, remembered);
      persisted = Boolean(session);
    } catch {
      // ignore
    }
    try {
      local?.setItem(storageKey, remembered);
      persisted = Boolean(local) || persisted;
    } catch {
      // ignore
    }
    if (persisted) inMemoryClientSessionIds.delete(storageKey);
    return remembered;
  }
  ...
  if (persisted) inMemoryClientSessionIds.delete(storageKey);
  else inMemoryClientSessionIds.set(storageKey, generated);
  return generated;
}
...
/** Resolve a writer id for this browser without adopting a server id. */
export function ensureBrowserClientSessionIdForAttempt(attempt: StudentAttempt): string {
  // Attempt projections can contain the active writer's last observed id.
  // Treating it as a fallback would let a fresh browser impersonate that
  // writer after local storage is cleared. Only browser-owned storage (or an
  // id already generated by this JS runtime) may establish this identity.
...
class BackendStudentAttemptRepository implements IStudentAttemptRepository {
  ...
  async getAttemptByScheduleId(
    ...
    const clientSessionId = ensureClientSessionId(scheduleId, studentKey);
    const session = await backendGet<BackendStudentSessionContext>(
      studentSessionTransport.paths.session(scheduleId, candidateId, clientSessionId)
    );
    if (!session.attempt) {
      return null;
    }
