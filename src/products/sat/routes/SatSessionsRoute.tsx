              ...
              <SatListRow key={summary.schedule.id} index={Math.min(rowIndex, 5)} onOpen={() => navigate('/sat/sessions/' + summary.schedule.id)}>
                <span className="flex w-full items-center gap-3 py-3">
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block truncate text-[10px] text-slate-400">{summary.schedule.cohortName}{summary.schedule.institution ? ' · ' + summary.schedule.institution : ''} · {satPublishScopeCopy(summary.schedule.publishScope ?? 'full')}</span>
...
function NewSatSessionSheet({ exams, examsLoading, saving, onClose, onGoToExamLibrary, onCreate }: { exams: Array<{ id: string; title: string; currentPublishedVersionId: string | null; currentPublishedScope?: 'full' | 'reading-writing' | 'math' | null | undefined }>; examsLoading: boolean; saving: boolean; onClose: () => void; onGoToExamLibrary: () => void; onCreate: (schedule: ExamSchedule) => Promise<void> }) {
