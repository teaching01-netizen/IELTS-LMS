...
export function SatSessionRoomRoute() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] text-slate-950">
      ...
      {confirm ? <SatConfirmDialog open title={confirm === 'complete' ? 'Finish this SAT session?' : `End ${selectedStudent?.name ?? 'this student'}’s attempt?`} description={confirm === 'complete' ? 'The session will be completed for the cohort. This should only be used when testing is finished.' : 'This ends the student’s current attempt. Their recorded answers remain available.'} confirmLabel={confirm === 'complete' ? 'Finish Session' : 'End Attempt'} destructive onCancel={() => setConfirm(null)} onConfirm={() => { const action = confirm; setConfirm(null); if (action === 'complete') void run('complete', () => controller.handleCompleteExam(scheduleId), 'Session completed.'); else if (selectedStudent) void runStudentAction('student-terminate', () => examDeliveryService.terminateStudentAttempt(selectedStudent.id, proctorName), `${selectedStudent.name}’s attempt ended.`); }} /> : null}
    </div>
  );
}
