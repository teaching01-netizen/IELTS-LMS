...
describe("useProctorRouteController backend mode", () => {
  ...
  it("surfaces failed cohort runtime mutations instead of reporting success", async () => {
    vi.stubEnv("VITE_FEATURE_USE_BACKEND_PROCTORING", "true");
    ...
    const startSpy = vi
      ...
      .mockResolvedValue({ success: false, error: "runtime unavailable" });
    const pauseSpy = vi
      .spyOn(examDeliveryService, "pauseRuntime")
      .mockResolvedValue({ success: false, error: "pause rejected" });
    const resumeSpy = vi
      .spyOn(examDeliveryService, "resumeRuntime")
      .mockResolvedValue({ success: false, error: "resume rejected" });
    ...
    await act(async () => {
      await expect(result.current.handleStartScheduledSession("sched-1")).rejects.toThrow(
        "runtime unavailable"
      );
      await expect(result.current.handlePauseCohort("sched-1")).rejects.toThrow("pause rejected");
      await expect(result.current.handleResumeCohort("sched-1")).rejects.toThrow("resume rejected");
    });

    expect(startSpy).toHaveBeenCalledWith("sched-1", "Proctor");
    expect(pauseSpy).toHaveBeenCalledWith("sched-1", "Proctor");
    expect(resumeSpy).toHaveBeenCalledWith("sched-1", "Proctor");
  ...
  it("keeps completed and cancelled summaries hydrated for the overview Past bucket", async () => {
    ...
    await waitFor(() => {
      expect(result.current.isLoading).toBe(false);
      expect(result.current.schedules.map((schedule) => schedule.status)).toEqual([
        "completed",
        "cancelled",
      ]);
    });

    expect(result.current.runtimeSnapshots.map((runtime) => runtime.status)).toEqual([
      "completed",
      "cancelled",
    ]);
  ...
  it("evaluates enabled violation rules, dispatches actions, and reloads monitoring state (happy path)", async () => {
    ...
    await waitFor(() => {
      expect(result.current.alerts.some((alert) => alert.type === "RULE_NOTIFY_PROCTOR")).toBe(
        true
      );
    });
