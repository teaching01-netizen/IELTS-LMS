...
import { hasBackendStatusCode } from "../api/backendStatus";
...
function persistAccessLinkQueue(
  linkId: string,
  ticket: StudentQueuedAdmission,
  form: AccessForm
): void {
...
export function StudentAccessLinkEntryRoute() {
  ...
  const [queued, setQueued] = useState<StudentQueuedAdmission | null>(
    () => restoredQueue?.ticket ?? null
  );
  const [queuedForm, setQueuedForm] = useState<AccessForm | null>(
    () => restoredQueue?.form ?? null
  );
  ...
  const locatorForThisLink = Boolean(
    link &&
    matchesSatResumeLocator(resumeLocator, { scheduleId: link.scheduleId, accessLinkId: link.id })
  );
  ...
  const finishEntry = useCallback(
    (
      activeLink: PublicStudentAccessLink,
      submitted: AccessForm,
      result: {
        scheduleId: string;
        studentCode: string;
  ...
  const submitEntry = useCallback(
    async (activeLink: PublicStudentAccessLink, submitted: AccessForm) => {
          ...
          if (!cancelled) {
            const message =
              error instanceof Error
                ? error.message
                : "Unable to continue from the admission queue.";
  ...
  if (
    linkQuery.isLoading &&
    resumeLocator?.providerKey === "sat" &&
    resumeLocator.accessLinkId === accessLinkId
  )
    return <SatLoadingSurface label="Checking your Student Link…" />;
    ...
    return (
      <EntryShell>
        <div className="flex min-h-72 flex-col items-center justify-center">
          ...
          <p className="mt-3 text-sm font-medium text-slate-500">Opening your exam…</p>
        </div>
      </EntryShell>
    );
  if (linkQuery.error || !link)
    return (
      <EntryShell>
        <UnavailableState
          icon={<AlertCircle size={24} />}
          title="This Student Link isn't available"
          description={
            linkQuery.error instanceof Error
              ? linkQuery.error.message
              : "Ask your teacher for a current link."
          }
        />
      </EntryShell>
    );
    ...
    <EntryShell>
      <div className="p-6 sm:p-8">
        ...
        {scopeCopy ? (
          <div
            role="note"
            className="mt-4 flex items-start gap-2 rounded-xl border border-[#0071e3]/15 bg-[#0071e3]/5 px-3 py-2.5 text-[11px] font-medium leading-4 text-[#0b5cad]"
          >
        ...
        {link.audienceLabel ? (
          <div className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-[#f5f5f7] px-2.5 py-1.5 text-[10px] font-semibold text-slate-600">
            <LockKeyhole size={11} />
            {link.audienceLabel}
          </div>
        ) : null}
        ...
        {submitError ? (
          <div
            role="alert"
            className="mt-5 rounded-xl border border-red-100 bg-red-50 px-3 py-2.5 text-[11px] font-medium text-red-700"
          >
            {submitError}
          </div>
        ) : null}
        {queued ? (
          <div
            aria-live="polite"
            className="mt-5 rounded-xl border border-blue-100 bg-blue-50 px-3 py-3"
          >
            <p className="text-[11px] font-semibold text-blue-800">You're in the admission queue</p>
            ...
            <button
              ...
              className="mt-2 text-[10px] font-semibold text-blue-700 underline hover:text-blue-900"
            >
              Leave queue
            </button>
          </div>
        ...
        {queuePollFailure ? (
          <div
            aria-live="polite"
            className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-3"
          >
            ...
            <div className="mt-2 flex gap-2">
              <button
                ...
                className="rounded-lg bg-[#0071e3] px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-[#0077ed]"
              >
                Retry
              </button>
              <button
                type="button"
                onClick={handleLeaveQueue}
                className="rounded-lg border border-black/[0.09] bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600"
              >
                Leave queue
              </button>
            </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          {link.accessMode === "student_code" ? (
            <InputField
              ...
              disabled={submitting || Boolean(queued)}
              onChange={(value) => {
          ...
          <button
            type="submit"
            disabled={submitting || Boolean(queued)}
            className="mt-2 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0071e3] px-5 text-sm font-semibold text-white transition-colors hover:bg-[#0077ed] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
          >
            {queued ? (
              "Waiting…"
            ) : submitting ? (
        ...
        <div className="mt-6 flex items-start gap-2 border-t border-black/[0.055] pt-4 text-[9px] leading-4 text-slate-400">
          <CheckCircle2 size={12} className="mt-0.5 shrink-0 text-emerald-500" />
          <span>
            This link is pinned to published Version {link.versionNumber}. Your teacher can pause or
            revoke this link without changing the exam itself.
          </span>
        </div>
      </div>
    </EntryShell>
...
function EntryShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#f5f5f7] px-4 py-8 text-slate-950 sm:py-14">
      <main className="mx-auto w-full max-w-[460px] overflow-hidden rounded-[24px] border border-black/[0.07] bg-white shadow-[0_16px_55px_rgba(0,0,0,0.08)]">
        {children}
      </main>
    </div>
  );
  ...
  const content =
    link.status === "upcoming"
      ? {
          ...
          description: link.opensAt
            ? `You can enter ${formatPublicDateTime(link.opensAt)}.`
            : "Come back when your teacher opens this link.",
          icon: <Clock3 size={24} />,
        }
      : link.status === "ended"
        ? {
            title: "This link has ended",
            description: link.closesAt
              ? `Student entry closed ${formatPublicDateTime(link.closesAt)}.`
              : "Ask your teacher if you still need access.",
            icon: <CheckCircle2 size={24} />,
          }
        : link.status === "paused"
          ? {
              title: "Entry is temporarily paused",
              description: "Your teacher can reopen this same link. You don’t need a new URL.",
              icon: <Clock3 size={24} />,
            }
          : {
...
function UnavailableState({
  ...
  return (
    <div className="p-7 text-center sm:p-9">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#f5f5f7] text-slate-500">
        {icon}
      </div>
      {eyebrow ? <p className="mt-5 text-[10px] font-semibold text-slate-400">{eyebrow}</p> : null}
      <h1 className="mt-2 text-[22px] font-semibold tracking-[-0.03em]">{title}</h1>
    ...
    <div>
      <label htmlFor={id} className="block text-[11px] font-semibold text-slate-700">
        ...
        <input
          ...
          className={`mt-1.5 block h-12 w-full rounded-xl border bg-white px-3 text-sm font-normal outline-none transition focus:ring-4 focus:ring-[#0071e3]/10 ${error ? "border-red-300" : "border-black/[0.09] focus:border-[#0071e3]/35"}`}
        />
      </label>
      {error ? (
        <p id={`${id}-error`} className="mt-1 text-[10px] font-medium text-red-600">
          {error}
        </p>
      ) : null}
    </div>
