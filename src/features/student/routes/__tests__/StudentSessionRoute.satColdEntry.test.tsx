...
vi.mock('@student/hooks/useStudentSessionRouteData', () => ({
  useStudentSessionRouteData: () => ({
    ...
    satBootstrapSeed: null, providerKey: 'sat', isLoading: false, state: {},
    attemptSnapshot: { id: 'attempt', candidateId: 'candidate' },
  }),
}));
...
it('loads only the SAT branch on a cold SAT entry', async () => {
  render(
  ...
  expect(await screen.findByText('SAT cold entry')).toBeInTheDocument();
  expect(imports).toEqual({ sat: 1, ielts: 0 });
});
