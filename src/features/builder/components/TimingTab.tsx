  ...
  const timingValidation = useMemo(() => {
    ...
    sectionKeys.forEach((module) => {
      ...
      if (section.duration <= 0) {
        errors.push({
          field: `sections.${module}.duration`,
          message: `${section.label} duration must be greater than 0.`,
        });
      }

      if (section.gapAfterMinutes < 0) {
        errors.push({
          field: `sections.${module}.gapAfterMinutes`,
          message: `${section.label} gap cannot be negative.`,
        });
      }
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-3">
        <div
          className={`flex items-start justify-between gap-4 rounded-xl border p-4 ${config.general.type === "ACT" ? "border-blue-100 bg-blue-50/60" : "border-amber-100 bg-amber-50/60"}`}
        >
          <div>
            {config.general.type === "ACT" ? (
              <>
                ...
                <p className="mt-1 text-[11px] text-blue-800">
                  Admin can change the duration and permitted special-time settings using the
                  existing scheduling controls.
                </p>
              </>
            ) : (
              <>
                <p className="text-xs font-bold uppercase tracking-widest text-amber-800">
                  Authentic IELTS Mode
                </p>
                <p className="mt-1 text-sm font-semibold text-amber-950">
                  Lock timing and runtime controls to match official IELTS expectations.
                </p>
                <p className="mt-1 text-[11px] text-amber-800">
                  When enabled: fixed L/R/W durations, no gaps, auto-submit on time up, no cohort
                  pause, and no section extensions or proctor section overrides.
                </p>
              </>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {sectionPlan.sections.map((section) => (
            <div
              ...
              className="px-3 py-2 rounded-full bg-blue-50 text-blue-800 border border-blue-100 text-xs font-bold uppercase tracking-wider"
            >
              {section.label}
            </div>
          ))}
        ...
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden divide-y divide-gray-50 shadow-sm">
              ...
              <div key={m} className="p-4 space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-gray-400 uppercase tracking-wider">
                        {planItem
                          ? `Planned start offset ${planItem.startOffsetMinutes} min`
                          : "Projected from session start"}
                      </span>
                    </div>
                  </div>
                ...
                {m === "writing" && (
                  <div className="pl-2 space-y-3">
                    {"tasks" in section &&
                      section.tasks.map((task, idx: number) => (
                        <div
                          key={task.id}
                          className="grid grid-cols-[minmax(0,1fr)_120px_120px_auto] gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100 items-end"
                        >
                          <div>
                            <label className="block text-[9px] font-bold text-gray-400 uppercase mb-1">
                              Label
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                newTasks[idx] = {
                                  ...task,
                                  recommendedTime: parseInt(e.target.value),
                                };
                                updateSection("writing", { tasks: newTasks });
                              }}
                              className="w-full px-2 py-1 border border-gray-200 rounded text-[10px] outline-none"
                ...
                {m === "speaking" && (
                  <div className="pl-2 space-y-3">
                    {"parts" in section &&
                      section.parts.map((part, idx: number) => (
                        <div
                          key={part.id}
                          className="grid grid-cols-3 gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100"
                        >
          ...
          <div className="p-4 bg-blue-50/30 flex items-center justify-between border-t border-blue-50">
            ...
            <span className="text-sm font-bold text-blue-900">
              {sectionPlan.plannedDurationMinutes} min
            </span>
          </div>
        </div>
        <p className="text-xs text-gray-500 italic">
          Actual clock times are derived from session start.
        </p>
      </section>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
              ...
              <p className="text-sm font-semibold text-gray-900 capitalize">
                {config.delivery.transitionMode.replace(/_/g, " ")}
              </p>
            </div>
          </div>
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">
              Allowed Extension Minutes
            </p>
            {isIeltsMode ? (
        ...
        <div className="space-y-4 bg-white border border-gray-100 p-4 rounded-xl shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-gray-500">
                Server automatically advances the cohort when a section timer reaches 0
              </p>
            </div>
            <input
              type="checkbox"
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-gray-500">
                Prevent candidates from returning to finished sections
              </p>
            </div>
            <input
              type="checkbox"
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-gray-500">
                Pause and resume the active section without resetting the timer
              </p>
            </div>
            <input
              type="checkbox"
          ...
          {config.progression.showWarnings && (
            <div className="flex items-center justify-between pt-2 pl-4 border-l-2 border-blue-50">
              ...
              <input
                ...
                onChange={(e) =>
                  updateConfig("progression", { warningThreshold: parseInt(e.target.value) })
                }
