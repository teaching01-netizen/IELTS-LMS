...
export function QuestionImportSheet({
  ...
  return (
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[1.2fr_0.8fr]">
        <div className="flex min-h-0 flex-col border-r border-au-separator p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[11px] font-semibold text-slate-700">Spreadsheet data</span>
            <label
              htmlFor="sat-question-import-file"
              className="authoring-interactive flex cursor-pointer items-center gap-1.5 rounded-[9px] px-2.5 py-1.5 text-[11px] font-semibold text-slate-600 hover:bg-au-fill"
            >
          ...
          <textarea
            ...
            className="min-h-[420px] flex-1 resize-none rounded-[12px] border border-au-separator bg-au-fill p-3 font-mono text-[11px] leading-5 text-slate-800 outline-none transition focus:border-au-accent/30 focus:bg-au-surface focus:ring-4 focus:ring-au-accent/10"
          />
        ...
        <div className="min-h-0 overflow-y-auto p-4">
          <div className="grid grid-cols-3 gap-2">
            ...
            <Metric label="Ready" value={result.drafts.length} good />
            <Metric label="Issues" value={result.issues.length} bad={result.issues.length > 0} />
          </div>
          {overCapacity ? (
            <div className="mt-3 flex gap-2 rounded-[12px] bg-au-danger-tint p-3 text-[11px] leading-5 text-au-danger-text">
              <AlertCircle size={14} className="mt-0.5 shrink-0" aria-hidden="true" />
              <span>
                This module has room for {remainingCapacity} more question
                {remainingCapacity === 1 ? "" : "s"}, but {result.drafts.length} valid rows are ready.
              </span>
            </div>
          ) : null}
          {runtimeError ? (
            <div role="alert" className="mt-3 rounded-[12px] bg-au-danger-tint p-3 text-[11px] text-au-danger-text">
              {runtimeError}
            </div>
          ) : null}
          <div className="mt-4">
            <h3 className="text-[11px] font-semibold text-slate-800">Review</h3>
            {result.issues.length ? (
              <div className="mt-2 space-y-1.5">
                {result.issues.slice(0, 40).map((issue, index) => (
                  <div
                    key={`${issue.row}-${issue.field}-${index}`}
                    className="flex gap-2 rounded-[10px] bg-au-warning-tint px-2.5 py-2 text-[11px] leading-4 text-au-warning-text"
                  >
              ...
              <div className="mt-2 flex items-center gap-2 rounded-[12px] bg-au-success-tint p-3 text-[11px] font-medium text-au-success-text">
                <CheckCircle2 size={15} aria-hidden="true" />
                {result.drafts.length} question{result.drafts.length === 1 ? "" : "s"} ready to import
              </div>
            ) : (
      ...
      <footer className="flex items-center justify-between border-t border-au-separator px-5 py-3">
        <span className="text-[11px] text-slate-400">Invalid rows are never sent to the server.</span>
        <div className="flex gap-2">
          ...
          >
            {isImporting ? "Importing…" : `Import ${result.drafts.length} Ready`}
          </button>
        </div>
      </footer>
    </AuthoringDialog>
...
function Metric({
  ...
  return (
    <div className="rounded-[12px] bg-au-fill px-3 py-2.5">
      <p
        className={`text-[17px] font-semibold tabular-nums ${good ? "text-au-success-text" : bad ? "text-au-danger-text" : "text-slate-900"}`}
      >
        {value}
      </p>
