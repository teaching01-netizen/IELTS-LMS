...
export function QuestionImportSheet({ open, sectionKey, remainingCapacity, isImporting, onClose, onImport }: QuestionImportSheetProps) {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={authoringMotion.surface} className="fixed inset-0 z-[95] flex items-center justify-center bg-black/28 p-4 backdrop-blur-[3px]" role="dialog" aria-modal="true" aria-label="Paste or import SAT questions" onMouseDown={(event) => { if (event.target === event.currentTarget && !isImporting) onClose(); }}>
          <motion.div ref={surfaceRef} initial={{ opacity: 0, y: 12, scale: 0.99 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.99 }} transition={authoringMotion.settle} className="au-elevation-sheet flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[20px] border border-black/[0.08] bg-white">
            <div className="flex items-center justify-between border-b border-black/[0.055] px-5 py-4"><div><h2 className="text-[13px] font-semibold tracking-[-0.012em] text-slate-950">Paste questions</h2><p className="mt-0.5 text-[11px] text-slate-500">Paste from Excel / Google Sheets or choose a CSV/TSV file. Import is atomic.</p></div><button type="button" disabled={isImporting} onClick={onClose} className="authoring-interactive flex h-9 w-9 items-center justify-center rounded-full text-slate-400 hover:bg-black/[0.05] disabled:opacity-30" aria-label="Close import"><X size={15} aria-hidden="true" /></button></div>
