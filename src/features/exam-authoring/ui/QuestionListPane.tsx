            ...
            <div role="group" aria-label={`Question ${question.displayOrder + 1} answer key`} className="flex gap-0.5">
              {(["A", "B", "C", "D"] as const).map((option) => (
                <button
                  ...
                  className={`authoring-interactive h-6 w-6 rounded-[7px] text-[10px] font-bold transition ${question.answerKeyPreview === option ? "bg-au-success-text text-white" : "text-slate-400 opacity-0 hover:bg-black/[0.05] hover:text-slate-700 group-hover:opacity-100 focus-visible:opacity-100"}`}
                >{option}</button>
              ))}
            </div>
