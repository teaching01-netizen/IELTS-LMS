...
export function QuestionEditor({
  ...
  return (
    <article className="mx-auto w-full max-w-[900px] px-6 pb-24 pt-6 sm:px-9 sm:pt-8">
      <header className="mb-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h2 className="mt-1 text-[20px] font-semibold tracking-[-0.025em] text-slate-950">
              Edit question
            </h2>
          </div>
          <div className="relative flex shrink-0 items-center gap-1">
            <button
              type="button"
              onClick={onDuplicate}
              className="authoring-interactive flex h-9 items-center gap-1.5 rounded-full px-3 text-[11px] font-semibold text-slate-600 hover:bg-black/[0.045]"
            ...
            <button
              ...
              className="authoring-interactive flex h-9 w-9 items-center justify-center rounded-full text-slate-500 hover:bg-black/[0.045]"
              aria-label="More question actions"
            >
              <MoreHorizontal size={15} />
            </button>
            {moreOpen ? (
              <div className="absolute right-0 top-11 z-40 min-w-44 rounded-xl border border-black/[0.08] bg-white p-1.5 shadow-[0_12px_30px_rgba(0,0,0,0.14)]">
                <button
                  type="button"
                  onClick={() => {
                    setMoreOpen(false);
                  ...
                  className="flex h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left text-[11px] font-semibold text-slate-600 hover:bg-slate-50"
                >
                  <Save size={12} />
                  Save now
                </button>
                <button
                  ...
                  className="flex h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left text-[11px] font-semibold text-red-600 hover:bg-red-50"
                >
                  <Trash2 size={12} />
                  Delete question
                </button>
                ...
                ? answer.options.map((option, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`group flex items-start gap-2 rounded-[15px] border p-2 transition ${correct ? "border-emerald-200 bg-emerald-50/60" : "border-transparent hover:bg-black/[0.018]"}`}
                      >
                        <motion.button
                          type="button"
                        ...
                        <div className="min-w-0 flex-1">
                          <FastQuestionComposer
                            ...
                            richTriggerLabel={`Add equation, image, graph, or table to choice ${letter}`}
      ...
      <div className="sticky bottom-3 z-20 mt-8 flex items-center justify-between rounded-[16px] border border-black/[0.07] bg-white/92 px-3 py-2 shadow-[0_10px_32px_rgba(0,0,0,0.09)] backdrop-blur-xl">
        <div className="flex min-w-0 items-center gap-3">
          <SaveState status={saveStatus} verbose />
          <label
            htmlFor="sat-keep-metadata-next"
            className="hidden cursor-pointer items-center gap-1.5 text-[9px] font-medium text-slate-500 sm:flex"
          ...
          >
            <input
              ...
              className="h-3.5 w-3.5 rounded border-slate-300 accent-[#0071e3]"
            />
        ...
        <motion.button
          ...
          className="flex h-10 items-center gap-2 rounded-full bg-[#0071e3] px-4 text-[11px] font-semibold text-white hover:bg-[#0077ed] disabled:opacity-45"
  ...
  return (
    <section data-authoring-field={field}>
      <div className="mb-2 flex items-baseline justify-between gap-3">
        ...
        <span
          className={`text-[10px] ${priority ? "font-medium text-[#0066cc]" : "text-slate-400"}`}
        >
          {hint}
        </span>
      </div>
    ...
    >
      ...
      {starters.map((starter) => (
        <button
          ...
          className="rounded-full bg-black/[0.045] px-2.5 py-1.5 text-[10px] font-semibold text-slate-600 transition hover:bg-black/[0.075] hover:text-slate-900"
        >
          {starter.label}
        </button>
      ))}
...
function ResponseTypeButton({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      onClick={onClick}
      className={`min-h-8 rounded-full px-3 text-[10px] font-semibold transition ${active ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
    >
      {children}
    </button>
  );
