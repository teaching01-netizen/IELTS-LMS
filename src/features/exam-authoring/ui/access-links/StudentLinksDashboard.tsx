...
export function StudentLinksDashboard({ exam, overview, isLoading, error, onRefresh, onBackToRelease }: StudentLinksDashboardProps) {
  ...
  return (
    <div className="sat-product sat-authoring flex min-h-screen flex-col bg-au-fill text-slate-950">
...
function StudentLinksLoading({ onBack }: { onBack: () => void }) { return <div className="sat-product sat-authoring min-h-screen bg-au-fill p-6" role="status" aria-busy="true"><button type="button" onClick={onBack} className="text-sm text-slate-500">← Release</button><div className="mx-auto mt-16 max-w-5xl"><div className="h-8 w-48 animate-pulse rounded-lg bg-au-fill-strong"/><div className="mt-6 h-[620px] animate-pulse rounded-[22px] bg-au-surface"/></div></div>; }
function StudentLinksError({ error, onBack, onRetry }: { error: string; onBack: () => void; onRetry: () => Promise<unknown> }) { return <div className="sat-product sat-authoring min-h-screen bg-au-fill p-6"><div role="alert" className="mx-auto max-w-lg rounded-[20px] bg-au-surface p-6"><h1 className="text-base font-semibold">Student Access could not load</h1><p className="mt-2 text-sm text-slate-500">{error}</p><div className="mt-5 flex gap-2"><button type="button" onClick={onBack} className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-600">Release</button><button type="button" onClick={() => void onRetry()} className="flex items-center gap-1 rounded-xl bg-au-accent px-4 py-2 text-sm font-semibold text-white"><RefreshCw size={13} aria-hidden="true"/>Retry</button></div></div></div>; }
function NoPublishedVersion({ onBack }: { onBack: () => void }) { return <div className="sat-product sat-authoring min-h-screen bg-au-fill p-6"><div className="mx-auto mt-20 max-w-lg rounded-[22px] bg-au-surface p-7 text-center"><Link2 size={28} className="mx-auto text-slate-300" aria-hidden="true"/><h1 className="mt-4 text-lg font-semibold">Publish before creating Student Access</h1><p className="mt-2 text-sm text-slate-500">Student access always uses an immutable published release.</p><button type="button" onClick={onBack} className="mt-5 rounded-xl bg-au-accent px-4 py-2.5 text-sm font-semibold text-white">Return to Release</button></div></div>; }
