...
export function StudentLinksDashboard({ exam, overview, isLoading, error, onRefresh, onBackToRelease }: StudentLinksDashboardProps) {
  ...
  useEffect(() => {
    ...
    const closeMenu = (event: KeyboardEvent) => { if (event.key === "Escape") setMenuLinkId(null); };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-au-fill text-slate-950">
      <header className="sticky top-0 z-40 border-b border-au-separator bg-white/88 authoring-glass">
        <div className="mx-auto flex min-h-[64px] w-full max-w-[1440px] items-center gap-3 px-4 sm:px-6">
          <button type="button" onClick={onBackToRelease} className="flex min-h-10 items-center gap-1.5 rounded-xl px-2 text-[12px] font-semibold text-slate-500 hover:bg-au-fill hover:text-slate-900"><ArrowLeft size={15}/>Release</button>
          ...
          <button type="button" onClick={() => { setEditingLink(null); setEditorOpen(true); }} className="flex min-h-10 items-center gap-1.5 rounded-full bg-au-accent px-4 text-[11px] font-semibold text-white hover:bg-au-accent-hover"><Plus size={13}/>New Link</button>
        </div>
      </header>
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-[1440px] flex-1 flex-col px-4 py-6 sm:px-6">
        ...
        {actionError ? <div role="alert" className="mb-4 flex items-center justify-between rounded-xl border border-au-danger/15 bg-au-danger-tint px-3 py-2 text-[11px] font-medium text-au-danger-text"><span>{actionError}</span><button type="button" onClick={() => setActionError(null)} aria-label="Dismiss error"><XCircle size={14}/></button></div> : null}

        <div className="flex min-h-[620px] flex-1 overflow-hidden rounded-[22px] border border-au-separator bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          <section className="flex min-w-0 flex-[0_0_56%] flex-col border-r border-au-separator">
            <div className="border-b border-au-separator p-3">
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative min-w-[200px] flex-1"><Search size={13} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} aria-label="Search Student Links" placeholder="Search links" className="h-10 w-full rounded-xl bg-au-fill pl-9 pr-3 text-[11px] outline-none focus:bg-white focus:ring-4 focus:ring-au-accent/10"/></div>
          ...
          <section className="min-w-[360px] flex-1 bg-au-fill">{selectedLink ? <AccessLinkDetail link={selectedLink} currentVersionId={version.id} currentVersionNumber={version.versionNumber} activity={activityQuery.data ?? []} activityLoading={activityQuery.isLoading} onCopy={() => void copy(selectedLink)} onShare={() => setShareLink(selectedLink)} onEdit={() => { setEditingLink(selectedLink); setEditorOpen(true); }} onPresent={() => setPresentLink(selectedLink)} onCreateForCurrent={() => void duplicate(selectedLink, "current")} /> : <div className="flex h-full items-center justify-center p-8 text-center"><div><Link2 size={28} className="mx-auto text-slate-300"/><p className="mt-3 text-[12px] font-semibold text-slate-600">Select a Student Link</p><p className="mt-1 text-[10px] leading-5 text-slate-400">Details, activity, sharing, and lifecycle controls appear here.</p></div></div>}</section>
        </div>
      </main>
...
function AccessLinkRow({ link, selected, currentVersionId, menuOpen, onSelect, onCopy, onShare, onMenu, onEdit, onDuplicate, onPauseResume, onPresent, onRevoke }: { link: AssessmentAccessLink; selected: boolean; currentVersionId: string; menuOpen: boolean; onSelect: () => void; onCopy: () => void; onShare: () => void; onMenu: () => void; onEdit: () => void; onDuplicate: () => void; onPauseResume: () => void; onPresent: () => void; onRevoke: () => void }) {
  return <div className={`group relative mb-1 rounded-xl border transition ${selected ? "border-[#0071e3]/30 bg-[#0071e3]/[0.045]" : "border-transparent hover:bg-black/[0.02]"}`}>
    ...
    <div className="absolute right-2 top-9 flex items-center gap-0.5 opacity-100 transition"><button type="button" onClick={onCopy} aria-label={`Copy ${link.name} link`} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-slate-400 shadow-sm hover:text-slate-800"><Copy size={12}/></button><button type="button" onClick={onShare} aria-label={`Share ${link.name}`} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-slate-400 shadow-sm hover:text-slate-800"><Share2 size={12}/></button><button type="button" onClick={onMenu} aria-label={`More actions for ${link.name}`} aria-haspopup="menu" aria-expanded={menuOpen} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-slate-400 shadow-sm hover:text-slate-800"><Ellipsis size={13}/></button></div>
    {menuOpen ? <div role="menu" aria-label={`Actions for ${link.name}`} className="absolute right-2 top-[70px] z-30 w-48 rounded-xl border border-au-separator bg-white p-1.5 shadow-[0_12px_40px_rgba(0,0,0,0.16)]"><MenuAction icon={<Pencil size={12}/>} onClick={onEdit}>Edit Link</MenuAction><MenuAction icon={<Copy size={12}/>} onClick={onDuplicate}>Duplicate Link</MenuAction><MenuAction icon={<Presentation size={12}/>} onClick={onPresent}>Present to Students</MenuAction>{link.lifecycleState !== "revoked" ? <MenuAction icon={link.lifecycleState === "paused" ? <Play size={12}/> : <Pause size={12}/>} onClick={onPauseResume}>{link.lifecycleState === "paused" ? "Resume Link" : "Pause Link"}</MenuAction> : null}<div className="my-1 h-px bg-au-fill"/><MenuAction destructive icon={<XCircle size={12}/>} onClick={onRevoke} disabled={link.lifecycleState === "revoked"}>Revoke Link</MenuAction></div> : null}
  </div>;
}
...
function MetricCard({ value, label }: { value: number; label: string }) { return <div className="rounded-xl border border-au-separator bg-white p-3"><p className="text-xl font-semibold tabular-nums tracking-[-0.03em]">{value}</p><p className="mt-0.5 text-[8px] font-semibold text-slate-400">{label}</p></div>; }
...
function MenuAction({ icon, onClick, children, destructive = false, disabled = false }: { icon: React.ReactNode; onClick: () => void; children: React.ReactNode; destructive?: boolean; disabled?: boolean }) { return <button type="button" role="menuitem" disabled={disabled} onClick={onClick} className={`flex min-h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left text-[10px] font-medium ${destructive ? "text-red-600 hover:bg-red-50" : "text-slate-600 hover:bg-slate-100"} disabled:opacity-35`}>{icon}{children}</button>; }

function ConfirmDialog({ open, title, description, confirmLabel, destructive, onCancel, onConfirm }: { open: boolean; title: string; description: string; confirmLabel: string; destructive?: boolean; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <AnimatePresence>{open ? <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[140] flex items-center justify-center bg-black/20 p-4 authoring-glass"><motion.div ref={dialogRef} role="alertdialog" aria-modal="true" aria-labelledby="access-link-confirm-title" aria-describedby="access-link-confirm-description" initial={{ scale: 0.98, y: 6 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-sm rounded-[20px] border border-au-separator bg-white p-5 shadow-2xl"><h2 id="access-link-confirm-title" className="text-[15px] font-semibold text-slate-950">{title}</h2><p id="access-link-confirm-description" className="mt-2 text-[11px] leading-5 text-slate-500">{description}</p><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onCancel} className="min-h-10 rounded-xl px-4 text-[11px] font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button type="button" onClick={onConfirm} className={`min-h-10 rounded-xl px-4 text-[11px] font-semibold text-white ${destructive ? "bg-red-600 hover:bg-red-700" : "bg-[#0071e3]"}`}>{confirmLabel}</button></div></motion.div></motion.div> : null}</AnimatePresence>;
}
function StudentLinksLoading({ onBack }: { onBack: () => void }) { return <div className="min-h-screen bg-au-fill p-6"><button type="button" onClick={onBack} className="text-sm text-slate-500">← Release</button><div className="mx-auto mt-16 max-w-5xl"><div className="h-8 w-48 animate-pulse rounded-lg bg-slate-200"/><div className="mt-6 h-[620px] animate-pulse rounded-[22px] bg-white"/></div></div>; }
function StudentLinksError({ error, onBack, onRetry }: { error: string; onBack: () => void; onRetry: () => Promise<unknown> }) { return <div className="min-h-screen bg-au-fill p-6"><div className="mx-auto max-w-lg rounded-[20px] bg-white p-6"><h1 className="text-base font-semibold">Student Access could not load</h1><p className="mt-2 text-sm text-slate-500">{error}</p><div className="mt-5 flex gap-2"><button onClick={onBack} className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-600">Release</button><button onClick={() => void onRetry()} className="flex items-center gap-1 rounded-xl bg-au-accent px-4 py-2 text-sm font-semibold text-white"><RefreshCw size={13}/>Retry</button></div></div></div>; }
function NoPublishedVersion({ onBack }: { onBack: () => void }) { return <div className="min-h-screen bg-au-fill p-6"><div className="mx-auto mt-20 max-w-lg rounded-[22px] bg-white p-7 text-center"><Link2 size={28} className="mx-auto text-slate-300"/><h1 className="mt-4 text-lg font-semibold">Publish before creating Student Access</h1><p className="mt-2 text-sm text-slate-500">Student access always uses an immutable published release.</p><button type="button" onClick={onBack} className="mt-5 rounded-xl bg-au-accent px-4 py-2.5 text-sm font-semibold text-white">Return to Release</button></div></div>; }
