...
export function ValidationChecklist({ issues, onIssueSelect }: ValidationChecklistProps) {
  ...
  return (
    <section aria-label="Validation" className="rounded-lg border border-border bg-card p-4">
      ...
      <div
        role="status"
        className={`mt-3 flex items-start gap-2 rounded-md px-3 py-2.5 text-xs font-semibold ${isReady ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
      >
        ...
        <div className="mt-2 space-y-2" aria-label="Question validation issues">
          {ordered.map((item) => (
            <button
              ...
              className={`flex w-full items-start gap-2 rounded-md border px-3 py-2.5 text-left text-xs ${item.blocking ? "border-red-200 bg-red-50 text-red-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}
            >
