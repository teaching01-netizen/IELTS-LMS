...
export interface InspectorProps {open:boolean;modal:boolean;question:QuestionRevision;issues:AssessmentValidationIssue[];onChange:(q:QuestionRevision)=>void;onClose:()=>void;isPretest?:boolean|undefined;onPretestChange?:((next:boolean)=>void)|undefined;}
export function Inspector({open,modal,question,issues,onChange,onClose,isPretest,onPretestChange}:InspectorProps){
 ...
 if(!open)return null;
 const content=<div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5"><ClassificationFieldset question={question} onChange={onChange} issues={issues}/><ItemSettingsInspector question={question} issues={issues} onChange={onChange} isPretest={isPretest} onPretestChange={onPretestChange}/></div>;
