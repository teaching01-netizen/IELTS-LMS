...
export function ComposerToolbar({editor,capabilities:c,onOpenDialog,onTableMutation}:ComposerToolbarProps){
 ...
 if(context.kind==='table'){
  ...
  if(c.history)more.push({id:'undo',label:'Undo',disabled:!state.undo,onSelect:()=>{editor.chain().focus().undo().run();}},{id:'redo',label:'Redo',disabled:!state.redo,onSelect:()=>{editor.chain().focus().redo().run();}});
  row=<>{c.blockStyles?<select aria-label="Text style" className="sat-rich-editor__style-select" value={state.style} onChange={e=>{const ch=editor.chain().focus();if(e.target.value==='heading2')ch.setHeading({level:2}).run();else if(e.target.value==='heading3')ch.setHeading({level:3}).run();else ch.setParagraph().run();}}><option value="paragraph">Text</option><option value="heading2">Heading</option><option value="heading3">Subheading</option></select>:null}{button('Bold (⌘B)',()=>{editor.chain().focus().toggleBold().run();},<Bold size={15}/>,state.bold)}{button('Italic (⌘I)',()=>{editor.chain().focus().toggleItalic().run();},<Italic size={15}/>,state.italic)}{c.equation?button('Insert equation',()=>onOpenDialog('math'),<span className="flex items-center gap-1"><Sigma size={15}/>Math</span>):null}{insert.length?menu('Insert content',insert):null}{menu('More formatting',more,true)}</>;
 }
