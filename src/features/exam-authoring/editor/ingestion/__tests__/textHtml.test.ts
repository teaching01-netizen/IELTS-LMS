...
describe("parseTextHtml html", () => {
  ...
  it("preserves math-looking text byte-identical", () => {
    ...
    const text = para.kind === "paragraph" ? para.children.map((c) => (c.kind === "text" ? c.text : "")).join("") : "";
    expect(text).toBe(latex);
  });
  it("normalizes Word and Docs chrome to the same semantics", () => {
    const word = parseTextHtml({ kind: "html", html: '<p class="MsoNormal">Hi<o:p></o:p></p>' }, rich);
    const docs = parseTextHtml({ kind: "html", html: '<b id="docs-internal-guid"><span class="c1">Hi</span></b>' }, rich);
