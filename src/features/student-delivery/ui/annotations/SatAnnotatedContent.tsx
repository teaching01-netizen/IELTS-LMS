...
import {
  createSatAnnotationNodeId,
  type SatAnnotationRegion,
} from "../../domain/satAnnotationIdentity";
 ...
 */
export function SatAnnotatedContent({
  content,
  annotations,
  region,
  enabled,
  ...
  useEffect(() => {
    ...
    const observer =
      typeof ResizeObserver === "undefined" ? null : new ResizeObserver(measureNoteMarkers);
  ...
  const renderText = useMemo<StructuredTextRenderer>(() => {
    ...
    return ({ nodeId, blockText, text, startOffset }) => {
      ...
      return segments.flatMap((segment) => {
        ...
        const interactive =
          match !== undefined && view.openEditorActive && view.annotationModeEnabled;
          ...
          <span
            ...
            {...(interactive && match
              ? {
                  ...
                  "data-sat-annotation-active":
                    view.activeAnnotationId === match.id ? "true" : undefined,
                  "aria-label": label,
                  title: label,
                  className:
                    "rounded-[2px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sat-focus)]",
                  onPointerDown: (event: React.PointerEvent) => {
                    // The gesture origin must be refreshed on the mark itself:
                    // selection dismissal can unmount the document observer
                    // before this press reaches its release handlers.
                    markSatPointerDown(event.clientX, event.clientY);
                  },
            ...
            style={{
              ...
              ...(segment.underline
                ? {
                    ...
                    textDecorationStyle: match ? satAnnotationUnderlineStyle(match) : "solid",
                    textDecorationColor: "var(--sat-underline, currentColor)",
              ...
              ...(segment.highlight || segment.underline
                ? {
  ...
  return (
    <div>
      ...
      {limitNotice ? (
        <p
          role="alert"
          data-testid={"sat-annotation-limit-" + region}
          className="mb-2 rounded-[8px] border border-[var(--sat-danger)] bg-[var(--sat-surface)] px-3 py-2 text-[13px] font-medium text-[var(--sat-danger)]"
        >
