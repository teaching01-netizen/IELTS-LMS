...
const CONTROL_CLASS =
  "sat-touch-target grid shrink-0 place-items-center rounded-md text-slate-600 hover:bg-slate-200/70 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-slate-600 " +
  PRESSED_AND_FOCUSED;

const LABELLED_CONTROL_CLASS =
  "sat-touch-target inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 text-[12px] font-semibold text-slate-600 hover:bg-slate-200/70 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-slate-600 " +
  PRESSED_AND_FOCUSED;
...
export function SatImageStrip(props: SatImageStripProps) {
  ...
  return (
    <div
      ...
      className="flex min-h-11 w-full flex-wrap items-center gap-0.5 rounded-lg border border-slate-200 bg-slate-50 px-1 py-0.5 focus-visible:outline-none"
    >
      <button
        type="button"
        ...
        className={CONTROL_CLASS + " w-11"}
      >
        <ZoomOut className="h-[18px] w-[18px]" aria-hidden="true" />
      </button>
      <button
        type="button"
      ...
      <span
        ...
        className="sat-tabular min-w-[48px] shrink-0 px-1 text-center text-[12px] font-semibold text-slate-700"
      >
      ...
      >
        ...
        {SAT_COPY.imageViewer.resetZoom}
      </button>
      ...
      >
        {props.fullScreen ? (
          <Minimize2 className="h-[15px] w-[15px]" aria-hidden="true" />
        ) : (
          <Maximize2 className="h-[15px] w-[15px]" aria-hidden="true" />
        )}
        {props.fullScreen ? SAT_COPY.imageViewer.exitFullScreen : SAT_COPY.imageViewer.fullScreen}
      </button>
    </div>
  );
}
