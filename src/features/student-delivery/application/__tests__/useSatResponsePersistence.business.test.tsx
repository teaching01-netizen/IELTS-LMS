...
function gateway(saveResponse: SatDeliveryGateway['saveResponse']): SatDeliveryGateway {
  return {
    saveResponse,
    bootstrap: vi.fn(), startModule: vi.fn(), submitModule: vi.fn(), submitAssessment: vi.fn(),
  } as SatDeliveryGateway;
}

function draft(answer: string, markedForReview = false): SatQuestionResponseDraft {
  return {
    questionId: 'q1', answer, markedForReview,
...
function snapshot(answer: string, revision: number): AssessmentResponseSnapshot {
  return {
    id: 'response-1', moduleAttemptId: 'module-attempt', examQuestionId: 'q1',
    response: answer, markedForReview: false, eliminatedOptions: [],
    annotations: { version: 1, note: '' }, revision,
  };
}

function canonicalBootstrap(attemptId = 'attempt-terminal'): AssessmentDeliveryBootstrap {
  return {
    scheduleId: 'schedule', examId: 'exam', providerKey: 'sat', versionId: 'version',
    serverNow: '2026-08-30T02:00:01Z', candidateName: 'Candidate',
    scheduleRuntimeStatus: 'live', proctorStatus: 'active', proctorNote: null,
    deviceFingerprintHash: null, sections: [], result: null,
    timing: {
      authority: 'cohort_runtime', timingModel: 'cohort_stage_v2',
      stageKey: 'reading-writing:m2', stageStatus: 'live',
      serverNow: '2026-08-30T02:00:01Z', deadlineAt: '2026-08-30T02:32:01Z',
      remainingSeconds: 1_920, runtimeRevision: 8,
    },
    attempt: {
      id: attemptId,
      moduleAttempts: [{
        id: 'module-attempt', moduleId: 'module', state: 'locked', allocatedSeconds: 1_920,
        availableAt: '2026-08-30T01:28:00Z', startedAt: '2026-08-30T01:28:00Z',
        pausedAt: null, accumulatedPausedSeconds: 0, extensionSeconds: 0,
        deadlineAt: '2026-08-30T02:00:00Z', remainingSeconds: 0,
        completionReason: 'time_expired', rawCorrect: 0, operationalQuestionCount: 1,
        toolState: {}, revision: 2,
      }],
      responses: [],
    },
  };
...
function structuredConflict(reason: string): ApiClientError {
  return new ApiClientError({
    message: 'SAT response conflict', statusCode: 409, backendCode: 'ASSESSMENT_CONFLICT',
    backendDetails: { reason }, backendRequestId: 'request-1',
  });
}
...
describe('SAT response durability laws', () => {
  ...
  it('keeps the newer answer pending when an older in-flight save is acknowledged', async () => {
    ...
    const { result } = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt', gateway: stableGateway, onSavedRevision: vi.fn(),
    }));
    ...
    act(() => { result.current.save(draft('B')); });

    await act(async () => { first.resolve(snapshot('A', 1)); });
    await waitFor(() => expect(saveResponse).toHaveBeenCalledTimes(2));

    expect(result.current.pendingDrafts.q1?.answer).toBe('B');
    await act(async () => { second.resolve(snapshot('B', 2)); });
  ...
  it('recovers an unsent answer after the student process is restarted', async () => {
    ...
    const first = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt',
      gateway: gateway(saveWhileOffline), onSavedRevision: vi.fn(),
    }));
    act(() => {
    ...
    expect(window.localStorage.getItem(
      satResponseCheckpointStorageKey('schedule', 'attempt', 'q1'),
    )).toContain('"answer":"C"');
    first.unmount();

    online.mockReturnValue(true);
    const saveResponse = vi.fn<SatDeliveryGateway['saveResponse']>(async (_schedule, _attempt, _question, request) =>
      snapshot(String(request.response), request.revision + 1));
    const second = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt', gateway: gateway(saveResponse), onSavedRevision: vi.fn(),
    }));
    act(() => { second.result.current.hydrateRevisions([]); });
  ...
  it('treats a matching bootstrap snapshot as the acknowledgement lost with the prior response', async () => {
    ...
    const first = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt',
      gateway: gateway(firstSave), onSavedRevision: vi.fn(),
    }));
    act(() => { first.result.current.hydrateRevisions([]); first.result.current.save(draft('D')); });
    ...
    const second = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt', gateway: gateway(retry), onSavedRevision: vi.fn(),
    }));
    act(() => { second.result.current.hydrateRevisions([snapshot('D', 1)]); });
  ...
  it('tombstones a terminal old-stage response so it cannot poison later flushes', async () => {
    ...
    const { result } = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt-terminal',
      gateway: terminalGateway, onSavedRevision: vi.fn(),
    }));

    act(() => {
      result.current.hydrateBootstrap(canonical);
      result.current.save(draft('A'), {
        moduleAttemptId: 'module-attempt', stageKey: 'reading-writing:m1',
        runtimeRevision: 7, remainingSeconds: 0, interactionType: 'discrete',
      });
    });
  ...
  it('never restores one SAT attempt response into another attempt on the same browser', async () => {
    ...
    const first = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt-a',
      gateway: gateway(firstAttemptSave), onSavedRevision: vi.fn(),
    }));
    act(() => { first.result.current.hydrateRevisions([]); first.result.current.save(draft('private-A')); });
    await waitFor(() => expect(first.result.current.failureKind).toBe('offline'));
    ...
    const second = renderHook(() => useSatResponsePersistence({
      scheduleId: 'schedule', attemptId: 'attempt-b', gateway: gateway(otherAttemptSave), onSavedRevision: vi.fn(),
    }));
    act(() => { second.result.current.hydrateRevisions([]); });
