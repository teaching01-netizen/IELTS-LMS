...
import { saveSatResumeLocator, loadSatResumeLocator } from "../satResumeLocator";
...
vi.mock("@services/backendBridge", () => ({
  ...
  hasBackendStatusCode: (error: unknown, code: number) =>
    typeof error === "object" &&
    error !== null &&
    ((error as { status?: number }).status === code ||
...
vi.mock("@services/studentSessionTransport", () => ({
  studentSessionTransport: {
    paths: {
      resume: (scheduleId: string, clientSessionId: string) => {
...
describe("resumeSatStudentSession", () => {
  beforeEach(() => {
    ...
    mocks.ensureClientSessionIdForStudentKey.mockImplementation(
      (_schedule: string, key: string, preferred?: string | null) => preferred ?? `writer:${key}`
    );
  ...
  it("ignores a tampered stored candidate and refreshes for the canonical browser writer", async () => {
    ...
    expect(mocks.backendGet.mock.calls[0]?.[0]).toContain(
      "clientSessionId=writer%3Astudent-schedule-1-wrong-candidate"
    );
  ...
  it("creates a browser-owned writer id when no locator exists and does not adopt the server active writer", async () => {
    ...
    await expect(
      resumeSatStudentSession({ scheduleId: "schedule-1", locator: null })
  ...
  it("clears a locator only after the authenticated server reports no attempt", async () => {
    ...
    await expect(
      resumeSatStudentSession({ scheduleId: "schedule-1", locator: loadSatResumeLocator() })
