    ...
    stimulus.blocks.forEach((block, blockIndex) => {
      ...
      questions.forEach((question, questionIndex) => {
        ...
        if (!validSkillCategories.has(question.skillCategory ?? "")) {
          errors.push({
            field: `${questionPrefix}.skillCategory`,
        ...
        question.options.forEach((option, optionIndex) => {
          if (!option.text.trim()) {
            errors.push({
              field: `${questionPrefix}.options[${optionIndex}].text`,
              message: `Option ${optionIndex + 1} in question ${questionIndex + 1} is empty`,
              type: "error",
            });
          }
...
export const migrateExam = (exam: Exam): Exam => {
  ...
  if (!content || !content.config) {
    content = {
      ...content,
      config: createDefaultConfig(exam.type, exam.type as ExamConfig["general"]["preset"]),
    } as ExamState;
  }
