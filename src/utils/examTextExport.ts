...
import { getMultiSelectCorrectOptionIds, getMultiSelectSelectionLimit } from "./multiSelectMcq";
...
import { getMcqOptionLabel } from "./mcqOptionLabel";
...
function optionLetter(option: MCQOption, index: number): string {
  return getMcqOptionLabel(option, index);
}

function formatOptionWithLetter(option: MCQOption, index: number): string {
  return `${optionLetter(option, index)}. ${toPlainText(option.text)}`;
}
...
function buildMcqAnswerDisplay(options: MCQOption[], answerIds: string[]): string {
  ...
  return answerIds.map((id) => indexed.find((entry) => entry.id === id)?.display ?? id).join(" | ");
}
  ...
  switch (block.type) {
    case "MATCHING":
      if (block.headings.length > 0) {
        ...
        block.headings.forEach((heading) => {
          ...
          context.lines.push(`  - ${heading.id}${headingText ? `. ${headingText}` : ""}`);
        });
      }
      break;
    case "TABLE_COMPLETION":
      if (block.headers.length > 0) {
        context.lines.push(
          `Headers: ${block.headers.map((header) => toPlainText(header)).join(" | ")}`
        );
      }
  ...
  switch (block.type) {
    case "TFNG": {
      ...
      if (question) {
        pushQuestion(
          context,
          toPlainText(question.statement) || "(empty statement)",
    ...
    case "CLOZE": {
      ...
      if (question) {
        pushQuestion(
          context,
          toPlainText(question.prompt) || "(empty prompt)",
          resolveAcceptedAnswers(question).join(" | ")
        );
      }
      break;
    }
    case "MATCHING": {
      ...
      if (question) {
        pushQuestion(
          context,
          `Paragraph ${toPlainText(question.paragraphLabel) || question.id}`,
          mapMatchingHeadingDisplay(block, question.correctHeading)
        );
      }
      break;
    }
    case "MAP": {
      ...
      if (question) {
        pushQuestion(
          context,
          `${toPlainText(question.label) || question.id} (x:${question.x}, y:${question.y})`,
          toPlainText(question.correctAnswer)
        );
      }
      break;
    }
    ...
    case "SINGLE_MCQ": {
      ...
      const options =
        subQuestion && subQuestion.options.length > 0 ? subQuestion.options : block.options;
      context.lines.push(`Stem: ${toPlainText(stem)}`);
      options.forEach((option, index) => {
        context.lines.push(`  ${formatOptionWithLetter(option, index)}`);
      });
      const correctOption = options.find((option) => option.isCorrect);
      const answer = correctOption ? buildMcqAnswerDisplay(options, [correctOption.id]) : "";
    ...
    case "SENTENCE_COMPLETION": {
      for (const question of block.questions) {
        ...
        if (blanks.length > 0) {
          ...
          const answer = blanks
            .map((blank) => resolveAcceptedAnswers(blank).join(" | "))
            .join(" | ");
    ...
    case "DIAGRAM_LABELING": {
      ...
      if (label) {
        pushQuestion(
          context,
          toPlainText(label.prompt) || "Label",
          toPlainText(label.correctAnswer)
        );
      }
      break;
    }
    ...
    case "TABLE_COMPLETION": {
      ...
      if (cell) {
        pushQuestion(
          ...
          resolveAcceptedAnswers(cell).join(" | ")
        );
      }
      break;
    }
    case "NOTE_COMPLETION": {
      for (let questionIndex = 0; questionIndex < block.questions.length; questionIndex += 1) {
        ...
        const blankIndex = question.blanks.findIndex(
          (candidate) => candidate.id === unit.questionId
        );
        if (blankIndex >= 0) {
    ...
    case "MATCHING_FEATURES": {
      ...
      if (feature) {
        pushQuestion(
          context,
          toPlainText(feature.text) || feature.id,
          toPlainText(feature.correctMatch)
        );
      }
      break;
    }
...
export function buildExamTextExport(exams: Exam[], exportedAt: Date = new Date()): string {
  const lines: string[] = [
...
export function downloadExamTextExport(content: string, exportedAt: Date = new Date()): string {
