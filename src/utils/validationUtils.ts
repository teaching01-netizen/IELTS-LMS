...
function validateSingleMCQ(block: SingleMCQBlock): ValidationError[] {
  ...
  questions.forEach((question, questionIndex) => {
    ...
    if (!Array.isArray(question.options) || question.options.length < 2) {
      errors.push({
        field: optionsField,
        message: `Question ${questionIndex + 1} needs at least 2 options`,
      });
      return;
    }
    ...
    if (correctCount !== 1) {
      errors.push({
        field: optionsField,
        message: `Question ${questionIndex + 1} must have exactly one correct option`,
      });
    }
...
function validateShortAnswer(block: ShortAnswerBlock): ValidationError[] {
  ...
  block.questions?.forEach((q: ShortAnswerQuestion, index: number) => {
    if (!q.prompt || q.prompt.trim() === "") {
      errors.push({
        field: `question-${index}-prompt`,
        message: `Question ${index + 1} prompt is required`,
      });
    }
    if (resolveAcceptedAnswers(q).length === 0) {
      errors.push({
        field: `question-${index}-answer`,
        message: `Question ${index + 1} correct answer is required`,
      });
    }
  });
  ...
  block.questions?.forEach((q: SentenceCompletionQuestion, index: number) => {
    if (!q.sentence || q.sentence.trim() === "") {
      errors.push({
        ...
        message: `Sentence ${index + 1} text is required`,
      });
    }
    const placeholderCount = countBlankPlaceholders(q.sentence);
    if (placeholderCount === 0) {
      errors.push({
        field: `sentence-${index}-blanks`,
        message: `Sentence ${index + 1} must include at least one blank placeholder (____)`,
      });
    } else if (!q.blanks || q.blanks.length !== placeholderCount) {
      errors.push({
        field: `sentence-${index}-blanks`,
        message: `Sentence ${index + 1} blanks must match the number of ____ placeholders`,
      });
    }
    if (q.acceptAnyAnswerKey !== true) {
      q.blanks?.forEach((blank: SentenceBlank, blankIndex: number) => {
        if (resolveAcceptedAnswers(blank).length === 0) {
          errors.push({
            field: `sentence-${index}-blank-${blankIndex}`,
            message: `Blank ${blankIndex + 1} answer is required`,
          });
        }
      });
    }
    errors.push(...validateGroupedSlotScoring(q.blanks ?? [], `sentence-${index}`, "Blank"));
  });
...
function validateFlowChart(block: FlowChartBlock): ValidationError[] {
  ...
  block.steps?.forEach((step: FlowChartStep, index: number) => {
    ...
    if (!step.correctAnswer || step.correctAnswer.trim() === "") {
      errors.push({
        field: `step-${index}-answer`,
        message: `Step ${index + 1} answer is required`,
      });
    }
  });
...
function validateTableCompletion(block: TableCompletionBlock): ValidationError[] {
  ...
  const placeholderAnalysis = analyzeTablePlaceholders(
    block.rows ?? [],
    block.headers?.length ?? 0
  );
  if (placeholderAnalysis.slots.length === 0) {
    errors.push({
      field: "rows-placeholders",
  ...
  errors.push(...validateGroupedSlotScoring(canonicalCells, "table-cells", "Cell"));
...
function validateGroupedSlotScoring(
  ...
  groupedSlots.forEach((groupSlots, groupId) => {
    ...
    if (
      requiredCorrect === undefined ||
      !Number.isInteger(requiredCorrect) ||
      requiredCorrect < 1
    ) {
      errors.push({
        field: `${fieldPrefix}-group-${groupId}-required`,
...
function validateNoteCompletion(block: NoteCompletionBlock): ValidationError[] {
  ...
  block.questions?.forEach((q: NoteCompletionQuestion, index: number) => {
    ...
    if (placeholderCount === 0) {
      errors.push({
        field: `note-${index}-blanks`,
        message: `Note ${index + 1} must include at least one blank placeholder (____)`,
      });
    } else if (!q.blanks || q.blanks.length !== placeholderCount) {
      errors.push({
        field: `note-${index}-blanks`,
        message: `Note ${index + 1} blanks must match the number of ____ placeholders`,
      });
    }
    q.blanks?.forEach((blank: NoteBlank, blankIndex: number) => {
      if (resolveAcceptedAnswers(blank).length === 0) {
        errors.push({
          field: `note-${index}-blank-${blankIndex}`,
          message: `Blank ${blankIndex + 1} answer is required`,
        });
      }
    });
  ...
  block.items?.forEach((item: ClassificationItem, index: number) => {
    ...
    if (!item.correctCategory || !block.categories.includes(item.correctCategory)) {
      errors.push({
        ...
        message: `Item ${index + 1} must be assigned to a valid category`,
      });
    }
  });
...
function validateMatchingFeatures(block: MatchingFeaturesBlock): ValidationError[] {
  ...
  block.features?.forEach((feature: MatchingFeature, index: number) => {
    ...
    if (!feature.correctMatch || !block.options.includes(feature.correctMatch)) {
      errors.push({
        field: `feature-${index}-match`,
        message: `Feature ${index + 1} must match a valid option`,
      });
    }
  });
