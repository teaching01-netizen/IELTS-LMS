...
describe("isStudentHighlightToolContextActive", () => {
  it("allows unblocked idle Reading, Listening, and Writing exam contexts", () => {
    ...
    expect(
      isStudentHighlightToolContextActive({ ...base, module: "reading", phase: "post-exam" })
    ).toBe(false);
    expect(
      isStudentHighlightToolContextActive({
        ...base,
        module: "reading",
        blockingReason: "proctor_paused",
      })
    ).toBe(false);
    expect(
      isStudentHighlightToolContextActive({ ...base, module: "reading", submitConfirmOpen: true })
    ).toBe(false);
    expect(
      isStudentHighlightToolContextActive({ ...base, module: "reading", finalSubmitIdle: false })
    ).toBe(false);
  });
