...
describe('student footer overlay layout ownership', () => {
  ...
  it('keeps end clearance in one shared scroll-owner contract', () => {
    if (!existsSync(layoutContractPath)) {
      expect.fail('studentFooterOverlayLayout.ts must own footer scroll clearance');
      return;
    }
    ...
    expect(contract).toMatch(
      /paddingBottom:\s*'var\(--student-exam-footer-reserve\)'/,
    );
    expect(contract).toMatch(
      /scrollPaddingBottom:\s*'var\(--student-exam-footer-reserve\)'/,
    );
  });
