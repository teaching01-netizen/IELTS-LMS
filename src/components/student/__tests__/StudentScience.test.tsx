...
describe('StudentScience', () => {
  ...
  it('shows one stimulus with its ACT Science question and four answer choices', () => {
    ...
    expect(screen.getByText('Which conclusion is supported by the experiment?')).toBeInTheDocument();
  ...
  it('lets a student select one answer and replace it with another answer', () => {
    ...
    expect(onAnswerChange.mock.lastCall?.slice(0, 2)).toEqual([
      'science-question-1',
      'option-b',
    ]);
  ...
  it('limits choice elimination controls to ACT Science options A through D', () => {
    ...
    const options = [
      ...block.options,
      { id: 'option-e', text: 'Option E', isCorrect: false },
    ];
  ...
  it('keeps eliminated choices independent for each ACT Science question', () => {
    ...
    expect(within(secondQuestion).getByRole('button', { name: 'Eliminate option B' })).toHaveAttribute(
      'aria-pressed',
      'false',
    );
    ...
    expect(within(firstQuestion).getByRole('button', { name: 'Restore option B' })).toBeInTheDocument();
    expect(within(secondQuestion).getByRole('button', { name: 'Restore option B' })).toBeInTheDocument();
  });
