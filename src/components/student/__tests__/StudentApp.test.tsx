...
describe("StudentApp runtime-backed mode", () => {
  it("preserves native page zoom while keeping the exam page lock active", () => {
    const viewport =
      document.querySelector<HTMLMetaElement>('meta[name="viewport"]') ??
      document.head.appendChild(document.createElement("meta"));
    viewport.name = "viewport";
    const initialViewportContent = "width=device-width, initial-scale=1.0";
    viewport.content = initialViewportContent;
    ...
    expect(viewport.content).toBe(initialViewportContent);
    expect(viewport.content).not.toMatch(/maximum-scale/i);
    expect(viewport.content).not.toMatch(/user-scalable=no/i);
  ...
  it("keeps local objective text input stable during same-attempt refresh", async () => {
    ...
    expect((screen.getByLabelText("Answer for question 1") as HTMLInputElement).value).toBe(
      "LOCAL_TYPED"
    );
  });
  ...
  it("keeps local writing editor content stable during same-attempt refresh", async () => {
    ...
    expect(
      ((await screen.findByRole("textbox", { name: /writing response/i })) as HTMLTextAreaElement)
        .value
    ).toContain("LOCAL_TYPED");
  });
  ...
  it("preserves a writing task draft after navigating away and back in runtime-backed mode", async () => {
    ...
    await waitFor(() => {
      ...
      ).toBe("Task 1 draft before switch");
    });
  });
  ...
  it("does not show dropped-mutation reconciliation banner to students", () => {
    ...
    expect(
      screen.queryByText(/This section has ended\. Moving you to the next section\./i)
    ).not.toBeInTheDocument();
  ...
  it("blocks submission when unansweredSubmissionPolicy is block", async () => {
    ...
    render(
      <StudentAppWrapper state={examState} onExit={() => {}} attemptSnapshot={attemptSnapshot} />
    );
