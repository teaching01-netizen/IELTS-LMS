...
describe('StudentApp runtime-backed mode', () => {
  ...
  it('ignores touchend viewport-lock updates when selection touchend targets a text node in highlightable surfaces', async () => {
    const originalInnerWidth = Object.getOwnPropertyDescriptor(window, 'innerWidth');
    const originalInnerHeight = Object.getOwnPropertyDescriptor(window, 'innerHeight');
    const originalMatchMedia = window.matchMedia;
    const originalMaxTouchPoints = Object.getOwnPropertyDescriptor(window.navigator, 'maxTouchPoints');
    ...
    try {
      ...
      act(() => {
        textNode.dispatchEvent(new Event('touchend', { bubbles: true }));
      });
