...
describe('student interaction motion class contracts', () => {
  ...
  describe('StudentHeader', () => {
    ...
    it('sweeps the zoom controls, highlight tools, and chrome buttons with the recipe', () => {
      ...
      expect(screen.getByRole('button', { name: 'Choose highlight color' })).toHaveClass(pressRecipe);
      expect(screen.getByRole('button', { name: 'Erase highlights' })).toHaveClass(pressRecipe);
      expect(screen.getByRole('button', { name: 'Open accessibility settings' })).toHaveClass(pressRecipe);
      expect(screen.getByRole('button', { name: 'Open question navigator' })).toHaveClass(pressRecipe);
    });
  ...
  describe('CompactStudentHeader', () => {
    it('sweeps the tools trigger, sheet actions, and swatches with the recipe', () => {
      ...
      for (const name of ['Question navigator', 'Highlight', 'Erase highlights', 'Accessibility settings']) {
        expect(screen.getByRole('button', { name })).toHaveClass(pressRecipe);
    ...
    it('attaches the urgency cue and color transition to the compact timer pill only under five minutes', () => {
      ...
      expect(screen.getByTestId('student-header-timer-slot')).not.toHaveClass('student-timer-urgent');
  ...
  describe('CompactQuestionNavigation', () => {
    it('sweeps previous/next, the center label, and Finish with the recipe', () => {
      ...
      expect(screen.getByRole('button', { name: /open question navigator/i })).toHaveClass(pressRecipe);
      ...
      expect(screen.getByRole('button', { name: 'Previous question' })).toHaveClass('disabled:opacity-40');
    });
  });
