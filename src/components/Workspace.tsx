...
export function Workspace({
   ...
   */
  const stableSetState = useCallback((next: ExamState | ((previous: ExamState) => ExamState)) => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "workspace-question-builder-collapsed",
      isQuestionBuilderCollapsed.toString()
    );
    ...
    isPassageListCollapsed,
    isQuestionBuilderCollapsed,
    isStimulusPaneCollapsed,
    isQuestionFocusMode,
  ]);

  const activePassage = state.reading.passages.find(
    (passage) => passage.id === state.activePassageId
  );
  ...
  const startNumber = useMemo(() => {
    ...
    const passageIndex = state.reading.passages.findIndex(
      (passage) => passage.id === activePassage.id
    );
    let num = 1;
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50 p-8">
          <div className="max-w-md w-full bg-white border border-gray-100 rounded-2xl shadow-sm p-6 space-y-3">
            ...
            <p className="text-sm text-gray-600">
              No passages exist for this exam. Add a passage to continue building the reading
              module.
            </p>
            <button
              type="button"
  ...
  const handlePassageSelect = (passageId: string) => {
    void stableSetState((previous) =>
      previous.activePassageId === passageId
        ? previous
        : { ...previous, activePassageId: passageId }
    );
  };
  ...
  const handleMetadataSave = (
    metadataUpdates: Omit<PassageMetadata, "id" | "createdAt" | "usageCount">
  ) => {
  ...
  return (
    <div className="flex-1 flex overflow-x-auto overflow-y-hidden transition-opacity duration-200 relative">
      ...
      <div
        className={`flex-shrink-0 transition-all duration-300 ease-in-out ${isQuestionFocusMode || isPassageListCollapsed ? "w-0 overflow-hidden" : "w-64"}`}
      >
        <PassageListSidebar
      ...
      <div
        className={`flex-shrink-0 transition-all duration-300 ease-in-out ${isQuestionFocusMode || isStimulusPaneCollapsed ? "w-0 overflow-hidden" : "flex-1 min-w-0"}`}
      >
      ...
      <div
        className={`flex-shrink-0 transition-all duration-300 ease-in-out h-full ${isQuestionBuilderCollapsed ? "w-0 overflow-hidden" : isQuestionFocusMode ? "flex-1" : "w-[480px] min-w-[400px]"}`}
      >
        <QuestionBuilderPane
