...
import { buildQuestionTracebackGroups, type ObjectiveTracebackGroup } from "./gradingReviewUtils";
        ...
        {group.items.map((item, itemIndex) => {
          return (
            ...
            >
              ...
              ) : item.requiredCorrect !== undefined &&
                item.slotLabels &&
                    ...
                    <div className="mt-2 space-y-1">
                      {item.studentAnswerSlots.map((slotValue, slotIndex) => (
                        ...
                        >
                          ...
                          <QuestionStatusBadge
                            correctness={item.slotCorrectness?.[slotIndex] ?? null}
                          />
                        </p>
                      ))}
                    </div>
...
export function QuestionTracebackPanel({
  ...
  const unmappedAnswerKeys = useMemo(() => {
    ...
    const descriptorAnswerKeys = new Set(
      groups.flatMap((group) =>
        group.items
          .flatMap((item) =>
            item.answerKeys && item.answerKeys.length > 0 ? item.answerKeys : [item.answerKey]
          )
          .filter(Boolean)
      )
    );
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      {examError ? (
        <div
          className="px-6 py-5 border-b border-gray-200 bg-red-50 text-sm text-red-800"
          role="alert"
        >
      ...
      {!examError && !examLoading && schemaGroups.length === 0 && fallbackGroups.length === 0 ? (
        <div className="px-6 py-6 text-sm text-gray-700">
          ...
          <p className="mt-1 text-gray-600">
            The exam version loaded, but no questions were found for this section. Showing raw
            answers:
          </p>
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="mt-1">
                    Missing number range(s): {numberingGapSummary}. This traceback preserves
                    canonical numbering.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-medium">
                    {unmappedAnswerKeys.length} stored answer key(s) do not map to this loaded exam
                    schema.
                  </p>
                  <p className="mt-1">
