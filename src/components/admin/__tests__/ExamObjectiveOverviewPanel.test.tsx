...
describe("buildExamObjectiveOverviewRows", () => {
  ...
  test("shows all table-cell accepted answers and matches against the full key", () => {
    ...
    expect(rows).toHaveLength(1);
    expect(rows[0]?.correctAnswer).toBe("faces of china | FACES OF CHINA | Faces of China");
  ...
  test("shows persisted audit findings inside the overall answer check without mark-review controls", async () => {
    ...
    vi.mocked(gradingService.getObjectiveIntegrityOverview).mockImplementation(async () => {
    ...
    render(
      <ExamObjectiveOverviewPanel
        session={
          { examTitle: "IELTS Mock Test", id: "session-1", scheduleId: "schedule-1" } as never
        }
      />
    );
  ...
  test("does not claim that there are no problems when the audit reports unresolved data without details", async () => {
    ...
    expect(
      screen.queryByText("No audit problems found in the persisted grading results.")
  ...
  test("shows the last overall-check decision per answer group", async () => {
    ...
    const keptLine = await screen.findByText(
      (_content, node) =>
        node?.tagName === "SPAN" &&
        node.textContent === "Kept incorrect by Grace Grader · Jan 2, 2026"
    );
    expect(keptLine).toBeInTheDocument();
    const acceptedLine = screen.getByText(
      (_content, node) =>
        node?.tagName === "SPAN" && node.textContent === "Accepted by Grace Grader · Jan 3, 2026"
    );
  ...
  test("highlights only the case-different character in the raw student answer", async () => {
    ...
    expect(
      screen.getByText("Capitalization differs from the closest accepted answer.")
    ).toBeInTheDocument();
  ...
  test("uses the active objective-grading draft after an answer-key update", async () => {
    ...
    vi.mocked(examRepository.getVersionById).mockImplementation(
      async (versionId) =>
        ({
    ...
    expect(
      await screen.findByText(
        "No typed answers differ from their key only by letter case or whitespace."
      )
    ).toBeInTheDocument();
  });
  ...
  test("keeps raw casing variants in separate review groups", () => {
    ...
    expect(groups.map((group) => group.studentAnswer)).toEqual(
      expect.arrayContaining(["garden hall", "Garden hall", "Garden Hall"])
    );
  ...
  test("sets one answer group for the whole exam and adds the answer to the key", async () => {
    ...
    vi.mocked(gradingRepository.getSectionSubmissionsBySubmissionId).mockImplementation(
      async (submissionId) => [makeSection(submissionId)]
    );
  ...
  test("coalesces a burst of update notifications into a single background reload", async () => {
    ...
    await waitFor(() =>
      expect(gradingRepository.getSectionSubmissionsBySubmissionId).toHaveBeenCalledTimes(2)
    );
