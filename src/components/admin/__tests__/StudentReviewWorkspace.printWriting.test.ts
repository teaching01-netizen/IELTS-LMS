...
describe("StudentReviewWorkspace print writing layout", () => {
  test("uses compact A4 Arial print setup matching session writing print", () => {
    const source = readFileSync(resolve(__dirname, "../StudentReviewWorkspace.tsx"), "utf8");
    ...
    expect(source).toMatch(/<div className="writing-print-response">\s*\{task\.text\}\s*<\/div>/);
