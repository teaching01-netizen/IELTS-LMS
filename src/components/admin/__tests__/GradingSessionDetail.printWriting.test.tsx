...
describe("GradingSessionDetail print writing", () => {
  ...
  beforeEach(() => {
    ...
    (gradingRepository.getWritingSubmissionsBySubmissionId as any).mockImplementation(
      (submissionId: string) =>
        Promise.resolve([
          makeWritingTask(
            submissionId,
            "task1",
            submissionId === "sub-1" ? "Ada response text" : "Ben response text"
          ),
        ])
    );
  });
  ...
  test("prints writing document from grading session detail", async () => {
    ...
    const printAllWritingButton = await screen.findByRole("menuitem", {
      name: /print all writing/i,
    });
  ...
  test("downloads ACT Science answers and scores from the session export menu", async () => {
    ...
    await waitFor(() =>
      expect(gradingRepository.getSubmissionsBySession).toHaveBeenCalledWith("session-1")
    );
  ...
  test("keeps the IELTS section statuses for an IELTS submission", async () => {
    ...
    expect(
      screen.queryByRole("menuitem", { name: /act science answers & scores/i })
