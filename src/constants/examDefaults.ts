...
export const ALL_QUESTION_TYPES: ExamConfig["sections"]["reading"]["allowedQuestionTypes"] = [
  "TFNG",
  "CLOZE",
  "MATCHING",
  "MAP",
...
const cloneWritingTasks = (
  tasks: StandardsConfig["writingTasks"]
): StandardsConfig["writingTasks"] => ({
...
const buildWritingTasksFromStandards = (
  ...
  const defaultTask1: WritingTaskConfig = {
    id: "task1",
    label: "Task 1",
  ...
  const defaultTask2: WritingTaskConfig = {
    id: "task2",
    label: "Task 2",
  ...
  const currentTasks =
    tasks && tasks.length > 0 ? tasks.map((task) => ({ ...task })) : [defaultTask1, defaultTask2];
        ...
        tasks: [
          {
            id: "task1",
            label: "Task 1",
            taskType: type === "Academic" ? "task1-academic" : "task1-general",
            minWords: 150,
            recommendedTime: 20,
          },
          {
...
const normalizeModuleConfig = <T extends ExamConfig["sections"][ModuleType]>(
  base: T,
  ...
  if ("partCount" in base) {
    (normalized as T & { partCount: number }).partCount = coerceInt(
  ...
  if ("passageCount" in base) {
    (normalized as T & { passageCount: number }).passageCount = coerceInt(
...
export const normalizeExamConfig = (config?: DeepPartial<ExamConfig>): ExamConfig => {
  ...
  const normalized = {
    ...
    delivery: {
      ...
      ...config.delivery,
      allowedExtensionMinutes:
        config.delivery?.allowedExtensionMinutes ?? base.delivery.allowedExtensionMinutes,
    },
    scoring: {
      ...base.scoring,
      ...config.scoring,
